...
\fancyhead[R]{\small\textit{Phiên bản 2.1}}
\fancyfoot[C]{\thepage}
...
\begin{tabular}{ll}
\textbf{Phiên bản:} & 2.1 \\[0.3cm]
\textbf{Ngày:} & 26/04/2026 \\[0.3cm]
\textbf{Trạng thái:} & Enterprise review -- cần khắc phục rủi ro trước go-live \\[0.3cm]
...
\chapter*{Lịch Sử Chỉnh Sửa (Revision History)}
\addcontentsline{toc}{chapter}{Lịch Sử Chỉnh Sửa (Revision History)}

\small
\begin{longtable}{|L{2.4cm}|L{1.8cm}|L{9.6cm}|}
\hline
\textbf{Ngày} & \textbf{Phiên bản} & \textbf{Tóm tắt thay đổi logic và xác nhận tiêu chuẩn Enterprise} \\
\hline
\endfirsthead
\hline
\textbf{Ngày} & \textbf{Phiên bản} & \textbf{Tóm tắt thay đổi logic và xác nhận tiêu chuẩn Enterprise} \\
\hline
\endhead
21/04/2026 & 2.0 & Hoàn thiện baseline SRS cho hệ thống tuyển dụng ITing. \\
\hline
26/04/2026 & 2.1 & Cập nhật SRS theo diff logic mới: Auto AI Review, AI-assisted search, HR Candidate Search, AI Operations API, schema job embedding/working days. Đánh giá Enterprise: chưa đạt go-live Enterprise tuyệt đối; cần hardening bảo mật API AI, kiểm soát dữ liệu hồ sơ ứng viên, DTO hóa response, tối ưu truy vấn vector search và chuẩn hóa logging trước khi triển khai khách hàng chiến lược. \\
\hline
\end{longtable}
\normalsize
...
\begin{enumerate}
    ...
    \item Đăng tin tuyển dụng và nhận kết quả Auto AI Review; Admin có thể hậu kiểm khi cần.
...
Module này cho phép tất cả người dùng (bao gồm Guest chưa đăng nhập) tìm kiếm, lọc và xem chi tiết tin tuyển dụng. Hệ thống hỗ trợ tìm kiếm từ khóa truyền thống và chế độ AI-assisted search. Ở chế độ AI-assisted, hệ thống mở rộng từ khóa bằng Knowledge Graph/Gemini, dùng vector search và cross-encoder để boost/xếp hạng lại các kết quả phù hợp trong tập kết quả đã lọc.
...
\hline
isAiSearch & Boolean & Không & Bật chế độ AI-assisted search để mở rộng từ khóa, boost kết quả semantic và đánh dấu gợi ý AI \\
\hline
\end{longtable}
...
\hline
\endhead
title & String & Có & Tiêu đề vị trí (tối đa 150 ký tự) \\
\hline
position & String & Có & Tên vị trí chính thức \\
\hline
description & Text & Có & Mô tả công việc chi tiết \\
\hline
requirements & Text & Không & Yêu cầu ứng viên, khuyến nghị nhập để tăng chất lượng matching \\
\hline
...
\hline
PENDING & Vàng & Chờ hệ thống AI/Admin kiểm duyệt \\
\hline
ACTIVE & Xanh lá & Đang hiển thị công khai \\
\hline
REJECTED & Đỏ & Bị AI hoặc Admin từ chối \\
\hline
...
\begin{itemize}
    \item \textbf{PENDING $\rightarrow$ ACTIVE}: AI tự động phê duyệt hoặc Admin phê duyệt.
    \item \textbf{PENDING $\rightarrow$ REJECTED}: AI tự động từ chối hoặc Admin từ chối (kèm lý do).
    ...
    \item \textbf{REJECTED $\rightarrow$ PENDING}: HR chỉnh sửa và gửi duyệt lại; hệ thống chạy AI review tự động.
\end{itemize}

\subsection{Cơ Chế Auto AI Review}

Sau khi HR tạo tin mới, cập nhật các trường quan trọng hoặc gửi duyệt lại tin bị từ chối, Backend tự động gọi AI review. AI trả về kết quả phân tích và quyết định cuối cùng:
\begin{itemize}
    \item Nếu AI kết luận \texttt{APPROVE}, tin chuyển sang \texttt{ACTIVE}.
    \item Nếu AI kết luận \texttt{REJECT}, tin chuyển sang \texttt{REJECTED} và lưu lý do vào \texttt{reviewReason}.
    \item Nếu AI lỗi hoặc phản hồi không rõ ràng, hệ thống mặc định cho phép tin chuyển sang \texttt{ACTIVE} để không chặn luồng đăng tin của HR; trạng thái AI được lưu để Admin có thể hậu kiểm.
\end{itemize}
...
\textbf{Điều kiện tiên quyết}: HR là chủ tin tuyển dụng \\
\textbf{Phân loại}: Bắt buộc
\end{requirementbox}

Khi HR chỉnh sửa các trường quan trọng như tiêu đề, vị trí, mô tả, yêu cầu, phúc lợi, lương, địa điểm hoặc hạn nộp, tin được đưa về \texttt{PENDING} và kích hoạt Auto AI Review. Kết quả AI có thể tự động chuyển tin sang \texttt{ACTIVE} hoặc \texttt{REJECTED}. Admin vẫn có quyền hậu kiểm, đình chỉ hoặc chỉnh trạng thái khi phát hiện vi phạm.
...
\begin{enumerate}
    ...
    \item \textbf{AI Review}: Hệ thống có thể đã tự động chạy AI review khi HR tạo/cập nhật/gửi duyệt lại tin; Admin cũng có thể kích hoạt lại AI review thủ công (POST /api/admin/jobs/\{id\}/ai-review):
    \begin{itemize}
        \item AI phân tích ngôn từ, độ tin cậy, nội dung không phù hợp.
        \item AI trả về trạng thái, lý do và cảnh báo kiểm duyệt.
    \end{itemize}
    \item Admin hậu kiểm hoặc ghi đè quyết định khi cần:
    \begin{itemize}
...
\textbf{Actor}: Employer (HR) \\
\textbf{Endpoints}: POST /api/employers/candidates/search, GET /api/employers/candidates/\{candidateId\}/profile \\
\textbf{Phân loại}: Bắt buộc
\end{requirementbox}

HR có thể nhập từ khóa kỹ năng, vị trí, cấp bậc, kinh nghiệm, địa điểm, học vấn hoặc bối cảnh ngành nghề để tìm kiếm trong tập hồ sơ ứng viên trên hệ thống. Hệ thống kết hợp bộ lọc hồ sơ, embedding similarity, Knowledge Graph expansion và location proximity để sắp xếp ứng viên phù hợp nhất.

\subsection{Luồng Xem Chi Tiết Hồ Sơ Ứng Viên}

Khi HR chọn một ứng viên từ kết quả tìm kiếm hoặc danh sách ứng viên yêu thích, hệ thống cung cấp endpoint xem hồ sơ đầy đủ gồm thông tin cơ bản, kỹ năng, kinh nghiệm, học vấn, chứng chỉ, liên kết xã hội và portfolio. Dữ liệu trả về phải được đóng gói theo response model phục vụ màn hình nhà tuyển dụng và chỉ hiển thị các trường hồ sơ được phép công khai trong ngữ cảnh tuyển dụng.

\section{UC-AI-04: Kiểm Duyệt Tin Bằng AI}

\begin{requirementbox}{UC-AI-04: AI Review tin tuyển dụng}
\textbf{Actor}: Admin, Employer-triggered workflow, AI System \\
\textbf{Endpoints}: POST /api/admin/jobs/\{id\}/ai-review; Auto trigger sau khi HR tạo/cập nhật/gửi duyệt lại tin \\
\textbf{Phân loại}: Bắt buộc
\end{requirementbox}
...
Kết quả AI review được lưu vào các trường \texttt{aiReviewStatus} và \texttt{aiReviewReason}. Với luồng tự động, AI có thể chuyển trạng thái tin sang \texttt{ACTIVE} hoặc \texttt{REJECTED}. Với luồng Admin gọi thủ công, kết quả AI được dùng để hỗ trợ kiểm duyệt và có thể tự động cập nhật trạng thái nếu AI đưa ra \texttt{FINAL\_DECISION} rõ ràng.

\section{UC-AI-05: Vận Hành Embedding và ML Service}

\begin{requirementbox}{UC-AI-05: Quản trị embedding và kiểm tra ML service}
\textbf{Actor}: Admin, AI System \\
\textbf{Endpoints}: POST /api/ai/embedding/trigger, GET /api/ai/ml/health, POST /api/ai/ner/extract, GET /api/ai/kg/expand, GET /api/ai/kg/explain \\
\textbf{Phân loại}: Vận hành nội bộ
\end{requirementbox}

Các endpoint AI vận hành phục vụ tạo embedding cho job, kiểm tra trạng thái Python ML service, trích xuất kỹ năng và giải thích matching bằng Knowledge Graph. Các endpoint có side effect như trigger embedding chỉ được sử dụng cho mục đích quản trị/vận hành hoặc scheduler nội bộ.
...
% ========================
% CHAPTER 11: ENTERPRISE READINESS
% ========================
\chapter{Đánh Giá Enterprise Readiness}

\section{Kết Luận PM/Enterprise Client}

Dựa trên thay đổi logic mới nhất trong \texttt{thay\_doi.diff}, hệ thống đã có nhiều nền tảng phù hợp Enterprise như JWT, RBAC theo role, Bean Validation cho nhiều request DTO, Flyway migration, logging cho các luồng AI/search và phân lớp Controller--Service--Repository. Tuy nhiên, trạng thái hiện tại được đánh giá là \textbf{Chưa đạt go-live Enterprise tuyệt đối}. Hệ thống có thể tiếp tục dùng cho demo/UAT có kiểm soát, nhưng cần hardening trước khi triển khai cho khách hàng chiến lược.

\section{Đánh Giá Theo Tiêu Chuẩn Enterprise}

\begin{longtable}{|p{3.2cm}|p{3cm}|p{8cm}|}
\hline
\textbf{Tiêu chí} & \textbf{Trạng thái} & \textbf{Nhận xét PM/Enterprise Client} \\
\hline
\endfirsthead
\hline
\textbf{Tiêu chí} & \textbf{Trạng thái} & \textbf{Nhận xét PM/Enterprise Client} \\
\hline
\endhead
Bảo mật & Cần cải thiện & Hệ thống có JWT/RBAC và một số endpoint dùng \texttt{@PreAuthorize}. Tuy nhiên nhóm endpoint \texttt{/api/ai/**} có endpoint side-effect như trigger embedding nên phải giới hạn Admin/Internal. Endpoint xem full profile ứng viên cần kiểm soát phạm vi dữ liệu/consent rõ ràng. \\
\hline
Validation và dữ liệu nhạy cảm & Cần cải thiện & DTO tạo job đã có validation cho nhiều trường chính, nhưng một số dữ liệu đầu vào dạng map/payload cần DTO hóa và validate bằng Bean Validation. API không được trả trực tiếp Hibernate Entity cho client ở các response hồ sơ đầy đủ. \\
\hline
Hiệu năng và quy mô & Cần cải thiện & Search ứng viên có \texttt{join fetch} cho user/account/skills giúp giảm một phần N+1. Tuy nhiên vector search hiện tải toàn bộ active job có embedding vào memory, phù hợp dữ liệu nhỏ nhưng chưa đạt quy mô Enterprise; cần pgvector/FAISS/index chuyên dụng và pagination/rerank trước khi mở rộng. \\
\hline
Tính bảo trì & Đạt có điều kiện & Code nhìn chung theo layer Spring Boot, nhưng một số logic AI review, parsing response AI, scoring và fallback đang nằm trực tiếp trong service lớn. Cần tách policy/strategy/service nhỏ hơn để dễ test và kiểm soát thay đổi. \\
\hline
Logging và monitoring & Cần cải thiện & Có log cho AI review, KG expansion, vector search và cross-encoder. Không được log raw AI response nếu response có thể chứa nội dung nhạy cảm của job/CV; cần correlation id, audit log nghiệp vụ và metric cho AI latency/failure rate. \\
\hline
\end{longtable}

\section{Rủi Ro Cần Khắc Phục Trước Enterprise Go-Live}

\begin{itemize}
    \item Khóa các endpoint AI vận hành có side effect theo role Admin hoặc internal network.
    \item Chuyển response full profile sang DTO thuần, không trả trực tiếp entity/lazy relation.
    \item Bổ sung authorization/consent rule cho HR khi xem hồ sơ ứng viên ngoài danh sách đã ứng tuyển.
    \item Thay vector search in-memory bằng cơ chế vector index hoặc ML service có pagination và threshold cấu hình.
    \item Chuẩn hóa logging: không log raw payload, CV, token, email nhạy cảm hoặc nội dung AI đầy đủ; chỉ log id, trạng thái, mã lỗi và metadata an toàn.
    \item Viết test cho auto AI review, authorization candidate profile, AI endpoint security, salary/job validation và negative cases.
\end{itemize}
...
CREATE TABLE Notification (
    ...
    message     TEXT,
    is_read     BOOLEAN DEFAULT FALSE,
    ref_id      BIGINT,                  -- Reference id (jobId, appId,...)
    created_at  TIMESTAMP DEFAULT NOW()
);
\end{lstlisting}
...
CREATE TABLE otp_code (
    ...
    created_at  TIMESTAMP DEFAULT NOW(),
    expires_at  TIMESTAMP NOT NULL       -- TTL = 10 minutes
);
...
\hline
POST & /api/employers/candidates/search & HR & Tìm kiếm ứng viên bằng AI/filter \\
\hline
GET & /api/employers/candidates/\{candidateId\}/profile & HR & Xem hồ sơ ứng viên đầy đủ \\
\hline
\end{longtable}

\subsection{AI Operations APIs}
...
\hline
\endhead
GET & /api/ai/kg/expand & Public/Internal & Mở rộng từ khóa bằng Knowledge Graph \\
\hline
GET & /api/ai/kg/explain & Public/Internal & Giải thích matching kỹ năng \\
\hline
POST & /api/ai/embedding/trigger & Admin/Internal & Trigger batch embedding cho job \\
\hline
GET & /api/ai/ml/health & Admin/Internal & Kiểm tra ML service \\
\hline
POST & /api/ai/ner/extract & Admin/Internal & Trích xuất kỹ năng từ văn bản \\
\hline
\end{longtable}
...
\section{Lịch Sử Thay Đổi SRS}

Mỗi lần chỉnh sửa tài liệu \texttt{SRS\_FULL.tex}, người thực hiện phải ghi lại ngày chỉnh sửa, người thực hiện, phạm vi thay đổi và prompt/yêu cầu gốc tạo ra thay đổi.

\small
\begin{longtable}{|L{2.2cm}|L{2.1cm}|L{3.4cm}|L{5.4cm}|}
\hline
\textbf{Ngày} & \textbf{Người thực hiện} & \textbf{Phạm vi thay đổi} & \textbf{Prompt/Yêu cầu gốc} \\
\hline
\endfirsthead
\hline
\textbf{Ngày} & \textbf{Người thực hiện} & \textbf{Phạm vi thay đổi} & \textbf{Prompt/Yêu cầu gốc} \\
\hline
\endhead
26/04/2026 & Codex & Bổ sung quy định ghi log cho mọi lần sửa SRS & Mỗi lần sửa đổi file SRS phải ghi lại log trong SRS\_FULL.tex và lưu prompt/yêu cầu gốc. \\
\hline
26/04/2026 & Codex & Cập nhật Revision History đầu file và bổ sung đánh giá Enterprise Readiness từ góc nhìn PM/Enterprise Client & Đóng vai PM cấp cao và Enterprise Client; kiểm tra tiêu chuẩn Enterprise, đối chiếu nghiệp vụ với diff, cập nhật SRS và ghi log lịch sử chỉnh sửa. \\
\hline
26/04/2026 & Codex & Sửa cảnh báo LaTeX khi biên dịch SRS & Sửa lỗi Underfull/Overfull hbox ở bảng log, cảnh báo fancyhdr headheight và ghi lại prompt sửa lỗi biên dịch. \\
\hline
\end{longtable}
\normalsize
...
\textbf{Phiên bản 2.1 -- Cập nhật: 26/04/2026}
\end{center}
