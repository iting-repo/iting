...
async function mockApplicationListingApis(page, opts = {}) {
  ...
  // Candidate full profile — opened when CandidateDetailModal mounts.
  // Without this, 401 from axiosInstance interceptor redirects to /login.
  await page.route('**/api/employers/candidates/*/profile', async (route) => {
...
test.describe('Nhà tuyển dụng quản lý ứng viên cho một công việc', () => {
  ...
  test('tìm kiếm ứng viên theo tên', async ({ page }) => {
    ...
    // Wait initial list to render
    await expect(page.getByText('Nguyễn Văn A')).toBeVisible();

    await page.getByPlaceholder(/T.m theo t.n/i).fill('Trần');

    await expect.poll(() => captured.lastSearch?.keyword, { timeout: 5000 }).toBe('Trần');
    await expect(page.getByText('Trần Thị B')).toBeVisible();
    // After re-render the unmatched name should disappear
  ...
  test('chấp nhận một ứng viên gọi đúng API accept', async ({ page }) => {
    ...
    // Modal opens — find Chấp nhận button inside it
    const acceptBtn = page.getByRole('button', { name: /Ch.p nh.n/i });
    await expect(acceptBtn).toBeVisible();
    await acceptBtn.click();

    await expect.poll(() => captured.accept?.url, { timeout: 8000 }).toContain('/employer/applications/501/accept');
  });
  ...
  test('từ chối ứng viên với lý do gọi đúng API reject', async ({ page }) => {
    ...
    // Open reject modal — button text is exactly "Từ chối"
    const rejectBtn = page.getByRole('button', { name: /^T. ch.i$/i });
    await expect(rejectBtn).toBeVisible();
    await rejectBtn.click();

    // Reject modal heading
    const rejectHeading = page.getByRole('heading', { name: /T. ch.i .ng vi.n/i });
    await expect(rejectHeading).toBeVisible();

    // Note textarea + confirm button
    await page.locator('textarea').last().fill('Hồ sơ chưa khớp với yêu cầu vị trí.');
    await page.getByRole('button', { name: /X.c nh.n t. ch.i/i }).click();

    await expect.poll(() => captured.reject?.url, { timeout: 8000 }).toContain('/employer/applications/501/reject');
  });
  ...
  test('chọn nhiều ứng viên và từ chối hàng loạt', async ({ page }) => {
    ...
    // Modal title is "Từ chối 2 ứng viên"
    const bulkModal = page.locator('.fixed.inset-0').filter({
      has: page.getByRole('heading', { name: /T. ch.i 2 .ng vi.n/i }),
    });
    await expect(bulkModal).toBeVisible();
    await bulkModal.locator('textarea').fill('Vị trí đã đủ ứng viên phù hợp.');
    await bulkModal.getByRole('button', { name: /X.c nh.n t. ch.i/i }).click();
