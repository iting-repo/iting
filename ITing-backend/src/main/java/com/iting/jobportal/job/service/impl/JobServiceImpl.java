...
public class JobServiceImpl implements JobService {
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Không tìm thấy tin tuyển dụng với id: " + jobId));
  }
  ...
  private Company findCompanyOrThrow(Long hrAccountId) {
    ...
    return companyRepository
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Không tìm thấy công ty của tài khoản hiện tại"));
  }

  /**
   * Kiểm tra HR đang login có quyền thao tác trên job (job phải thuộc Company mà HR đã được duyệt
  ...
  private void checkOwnership(Job job, Long hrAccountId) {
    if (job.getCompany() == null || job.getCompany().getId() == null) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Tin tuyển dụng chưa gắn với công ty hợp lệ");
    }
  ...
  private void validateSalary(Job job) {
    if (job.getMinSalary() != null
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Lương tối thiểu không được lớn hơn lương tối đa");
    }
  }
  ...
  /**
   * Build searchable text from a Job for Cross-Encoder reranking. Combines title, position, tech
   * stack, and truncated description.
   */
  ...
  public JobSearchRequest analyzeCvFileForSearch(MultipartFile file) {
    ...
    try {
      ...
      if (extractionResult.isEmpty()) {
        log.error(
            "HF CV extraction returned empty result — service may be sleeping or unreachable");
        throw new ResponseStatusException(
            HttpStatus.BAD_GATEWAY,
            "Dịch vụ AI phân tích CV đang tạm ngưng hoạt động. Vui lòng thử lại sau hoặc dán nội"
                + " dung CV thủ công.");
      }
      ...
      if (!result.isSuccess()) {
        ...
        throw new ResponseStatusException(
            HttpStatus.BAD_GATEWAY,
            "Không thể phân tích file CV: "
                + (result.error() != null ? result.error() : "Lỗi không xác định"));
      }
      ...
      if (cvText == null || cvText.isBlank()) {
        ...
        throw new ResponseStatusException(
            HttpStatus.BAD_GATEWAY,
            "Không trích xuất được nội dung từ file CV. Vui lòng thử file khác hoặc dán nội dung CV"
                + " thủ công.");
      }
      return geminiService.extractSearchCriteriaFromCv(cvText);
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_GATEWAY, "Không thể phân tích CV qua AI. Vui lòng thử lại sau.");
    }
  }
  ...
  public JobResponse submitJobForReview(Long employerId, Long jobId) {
    ...
    if (job.getStatus() != JobStatus.REJECTED) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Chỉ có thể gửi duyệt lại khi tin đang bị từ chối");
    }
  ...
  public JobResponse createJob(Long employerId, CreateJobRequest request) {
    ...
    entityManager
        .createNativeQuery(
            "INSERT INTO company_upload_job (job_id, company_id, time) VALUES (:jobId, :companyId,"
                + " CURRENT_TIMESTAMP)")
    ...
    outboxAppender.ifPresent(
        appender ->
            appender.append(
                kafkaTopics.getJobEmbeddingRequested(),
                "job",
                JobEmbeddingRequestedEvent.of(
                    saved.getId(), saved.getTitle(), digestOf(saved.getDescription()))));
  ...
  public JobResponse updateJob(Long employerId, Long jobId, UpdateJobRequest request) {
    ...
    } else if (request.getProvince() != null
        || request.getProvince() != null
        || request.getAddress() != null) {
  ...
  public JobResponse extendJob(Long employerId, Long jobId, int days) {
    if (days <= 0 || days > 365) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Số ngày gia hạn phải từ 1 đến 365");
    }
    ...
    if (job.getStatus() == JobStatus.CLOSED) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Không thể gia hạn tin tuyển dụng đã đóng");
    }

    LocalDate baseDate =
        (job.getDueDate() == null || job.getDueDate().isBefore(LocalDate.now()))
            ? LocalDate.now()
            : job.getDueDate();
  ...
  public JobResponse closeJob(Long employerId, Long jobId) {
    ...
    if (job.getStatus() == JobStatus.CLOSED) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Tin tuyển dụng đã được đóng trước đó");
    }
  ...
  public JobResponse reopenJob(Long employerId, Long jobId) {
    ...
    if (job.getStatus() != JobStatus.CLOSED) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Chỉ có thể mở lại tin tuyển dụng đã đóng");
    }

    // (optional) check hạn
    if (job.getDueDate() == null || job.getDueDate().isBefore(LocalDate.now())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Tin tuyển dụng đã hết hạn, vui lòng gia hạn trước");
    }
  ...
  public Page<JobResponse> getJobsByEmployer(Long employerId, int page, int size) {
    ...
    return jobRepository.findByCompany_Id(company.getId(), pageable).map(this::enrichWithCompany);
  }
  ...
  public Page<JobResponse> searchJobs(JobSearchRequest request, Long userId) {
    ...
    if (request.getIsAiSearch() != null
        && request.getIsAiSearch()
    ...
    if (request.getIsAiSearch() != null
      ...
      try {
        ...
        List<Long> docIds =
            jobPage.getContent().stream().map(Job::getId).collect(Collectors.toList());
    ...
    List<JobResponse> content =
        jobPage.getContent().stream().map(this::enrichWithCompany).collect(Collectors.toList());
    ...
    boolean hasAiSuggestion =
        content.stream().anyMatch(j -> Boolean.TRUE.equals(j.getIsAiSuggested()));
  ...
  public List<JobResponse> getLatestJobs(int limit) {
    ...
    return jobRepository
        .findAllByStatus(JobStatus.ACTIVE, pageable)
  ...
  public List<JobResponse> getHotJobs(int limit) {
    ...
    return jobRepository
        .findHotJobs(JobStatus.ACTIVE, pageable)
        .map(this::enrichWithCompany)
        .getContent();
  }
  ...
  //                    "Công ty của bạn chưa được phê duyệt. Vui lòng hoàn tất hồ sơ công ty
  // trước."
  //            );
  //        }
  //
  ...
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Tiêu đề công việc không
  // được để trống");
  //        }
  //
  //        if (job.getPosition() == null || job.getPosition().isBlank()) {
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Vị trí tuyển dụng không
  // được để trống");
  //        }
  //
  //        if (job.getDescription() == null || job.getDescription().isBlank()) {
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Mô tả công việc không
  // được để trống");
  //        }
  //
  //        if (job.getProvince() == null || job.getProvince().isBlank()) {
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Thành phố không được để
  // trống");
  //        }
  //
  //        if (job.getAddress() == null || job.getAddress().isBlank()) {
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Địa chỉ không được để
  // trống");
  //        }
  //
  //        if (job.getMinSalary() == null || job.getMaxSalary() == null) {
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Thông tin lương không
  // được để trống");
  //        }
  //
  //        validateSalary(job);
  //
  //        if (job.getDueDate() == null || job.getDueDate().isBefore(LocalDate.now())) {
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Hạn ứng tuyển không hợp
  // lệ");
  //        }
  //
  //        if (job.getMaxAccept() == null || job.getMaxAccept() <= 0) {
  //            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Số lượng tuyển phải lớn
  // hơn 0");
  //        }
  //
  //        job.setStatus(JobStatus.PENDING);
  ...
  /**
   * Tự động chạy AI review cho job mới đăng hoặc gửi duyệt lại. AI sẽ quyết định APPROVE (ACTIVE)
   * hoặc REJECT. Nếu AI lỗi hoặc không phản hồi → mặc định APPROVE.
   */
  private void autoAiReview(Job job) {
    try {
      ...
      String normalized =
          reviewResult.replace("*", "").replace("\\", "").replace("\n", " ").toLowerCase();
      ...
      if (!isReject && !isApprove) {
        ...
        boolean hasRejectKeyword =
            normalized.contains("từ chối")
                || normalized.contains("tu choi")
                || normalized.contains("không đạt")
                || normalized.contains("không duyệt");
        boolean hasApproveKeyword =
      ...
      log.warn(
          "[AUTO-AI-REVIEW] Lỗi khi duyệt tự động job id={}: {}. Mặc định APPROVE.",
          job.getId(),
          e.getMessage());
      job.setStatus(JobStatus.ACTIVE);
  ...
  private void validateJobBeforeSubmit(Job job, Company company) {
    ...
    List<String> blacklist =
        List.of("cướp", "lừa đảo", "đánh bạc", "ma túy", "buôn lậu", "tống tiền");
    for (String word : blacklist) {
      if (title.contains(word) || description.contains(word)) {
        ...
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST,
            "Tin tuyển dụng bị hệ thống tự động từ chối do chứa nội dung không hợp lệ hoặc vi phạm"
                + " pháp luật.");
      }
    }
    ...
    if (job.getTitle() == null || job.getTitle().isBlank()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Tiêu đề công việc không được để trống");
    }

    if (job.getTitle().length() > 150) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Tiêu đề công việc không được vượt quá 150 ký tự");
    }

    if (job.getPosition() == null || job.getPosition().isBlank()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Vị trí tuyển dụng không được để trống");
    }

    if (job.getDescription() == null || job.getDescription().isBlank()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Mô tả công việc không được để trống");
    }
    ...
    if (job.getSalaryType() != SalaryType.NEGOTIABLE) {
      if (job.getMinSalary() == null || job.getMaxSalary() == null) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Thông tin lương không được để trống");
      }
      validateSalary(job);
    }
    ...
    if (job.getDueDate() == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Hạn ứng tuyển không được để trống");
    }

    if (job.getDueDate().isBefore(LocalDate.now())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Hạn ứng tuyển không được ở quá khứ");
    }
  ...
  public SalaryReportResponse getSalaryReport(String keyword, String location, String experience) {
    ...
    // 3. Fallback: Nếu không có kết quả, mở rộng tìm kiếm theo related keywords (nếu có hệ thống
    // mapping)
    ...
    if (keyword != null && !keyword.isBlank()) {
          ...
          (a, b) -> {
            long countA =
                java.util.Arrays.stream(tokens)
                    .filter(
                        t ->
                            (a.getPosition() != null && a.getPosition().toLowerCase().contains(t))
            ...
            long countB =
                java.util.Arrays.stream(tokens)
                    .filter(
                        t ->
                            (b.getPosition() != null && b.getPosition().toLowerCase().contains(t))
      ...
      jobs =
          jobRepository.findAll(
              com.iting.jobportal.job.repository.JobSpecification.fromRequest(request));
    }
    ...
    double totalAvg =
        salaryJobs.stream()
            .mapToDouble(
                j ->
                    (j.getMinSalary().doubleValue() + j.getMaxSalary().doubleValue())
                        / 2.0
                        / 1_000_000.0)
            .average()
            .orElse(0.0);
    ...
    java.util.Map<com.iting.jobportal.job.entity.enums.ExperienceLevel, Double> expMap =
        salaryJobs.stream()
            .collect(
    ...
    List<SalaryReportResponse.ChartData> experienceStats =
        expMap.entrySet().stream()
            .map(
                e ->
                    new SalaryReportResponse.ChartData(
                        formatExperienceLabel(e.getKey()), Math.round(e.getValue() * 10.0) / 10.0))
            .collect(Collectors.toList());

    // 3. Thống kê theo khu vực (Top 5)
    java.util.Map<String, Double> locMap =
        salaryJobs.stream()
    ...
    List<SalaryReportResponse.ChartData> locationStats =
        locMap.entrySet().stream()
            .map(
                e ->
                    new SalaryReportResponse.ChartData(
                        e.getKey(), Math.round(e.getValue() * 10.0) / 10.0))
            .sorted((a, b) -> b.getValue().compareTo(a.getValue()))
            .limit(5)
            .collect(Collectors.toList());

    // 4. Top jobs lương cao (Top 4)
    List<JobResponse> highSalaryJobs =
        salaryJobs.stream()
    ...
    List<String> relatedPositions =
        jobs.stream()
            .map(j -> j.getPosition() != null ? j.getPosition() : j.getTitle())
            .filter(
                p ->
                    p != null
                        && (keyword == null || !p.toLowerCase().contains(keyword.toLowerCase())))
            .distinct()
            .limit(6)
            .collect(Collectors.toList());
