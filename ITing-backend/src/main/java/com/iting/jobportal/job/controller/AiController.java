...
@Tag(
    name = "09. AI Services",
    description = "APIs cho Knowledge Graph, Embedding, NER và Explainability")
public class AiController {
  ...
  public ResponseEntity<Map<String, Object>> explainMatch(
      @RequestParam List<String> cvSkills, @RequestParam List<String> jdSkills) {
  ...
  public ResponseEntity<Map<String, Object>> mlHealth() {
    ...
    return ResponseEntity.ok(
        Map.of("mlServiceAvailable", available, "status", available ? "CONNECTED" : "UNAVAILABLE"));
  }
  ...
  public ResponseEntity<Map<String, Object>> extractSkills(@RequestBody Map<String, String> body) {
    ...
    return ResponseEntity.ok(
        Map.of(
            ...
            skills.size(),
            "source",
            mlServiceClient.isAvailable() ? "ML_SERVICE" : "UNAVAILABLE"));
  }
}
