...
public class JobAlertServiceImpl implements JobAlertService {
  ...
  @Override
  public Page<FollowedCompanyJobResponse> getJobsFromFollowedCompanies(
      Long userId, Pageable pageable) {
    List<UserFollowCompany> followedCompanies =
        userFollowCompanyRepository.findByUserId(userId, Pageable.unpaged()).getContent();
    ...
    for (Long companyId : companyIds) {
      Page<Job> companyJobs =
          jobRepository.findByCompany_IdAndStatus(companyId, JobStatus.ACTIVE, Pageable.unpaged());
    ...
    List<FollowedCompanyJobResponse> responses =
        pagedJobs.stream().map(job -> toResponse(job, userId)).collect(Collectors.toList());
