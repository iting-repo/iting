...
public class UserJobController {
  ...
  public ResponseEntity<JobResponse> getJob(@CurrentUser Long userId, @PathVariable Long id) {
    if (userId != null) {
      interactionService.trackInteraction(
          userId, id, com.iting.jobportal.recommendation.entity.enums.InteractionType.VIEW);
    }
  ...
  public com.iting.jobportal.job.dto.request.JobSearchRequest analyzeCv(
      @RequestBody String cvText) {
    return jobService.analyzeCvForSearch(cvText);
  }

  @PostMapping(
      value = "/analyze-cv-file",
      consumes = org.springframework.http.MediaType.MULTIPART_FORM_DATA_VALUE)
  @Operation(
      summary = "Phân tích CV file (PDF/Image) qua AI để tìm việc phù hợp",
  ...
  @Operation(
      summary = "🤖 AI Gợi ý: Việc làm phù hợp với CV của bạn",
      description =
          "Dựa trên CV embedding của ứng viên, trả về top N jobs có cosine similarity cao nhất")
