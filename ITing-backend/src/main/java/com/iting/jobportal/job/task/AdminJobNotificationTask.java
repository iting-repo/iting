...
public class AdminJobNotificationTask {
  ...
  public void notifyAdminPendingJobs() {
    ...
    if (pendingCount > 0) {
      log.info(
          "[SCHEDULED TASK] Found {} pending jobs. Sending notifications to all admins.",
          pendingCount);
      ...
      for (Account admin : admins) {
        CreateNotificationRequest request =
            CreateNotificationRequest.builder()
                ...
                .content(
                    "Công việc đang chờ duyệt: Hiện đang có "
                        + pendingCount
        ...
        try {
          ...
          log.error(
              "Failed to send pending job notification to admin {}: {}",
              admin.getEmail(),
              e.getMessage());
        }
      }
