...
/**
 * Read-through cache for {@link RefreshToken} validation. The DB stays the source of truth
 * (revocation, rotation), but every refresh-flow lookup hits Redis first to spare a JPA query on
 * the hot auth path.
 *
 * <p>Invalidation is explicit: callers MUST evict on revoke/markAsUsed.
 */
@Service
...
public class RefreshTokenCacheService {
  ...
  private String userIndex(Long userId) {
