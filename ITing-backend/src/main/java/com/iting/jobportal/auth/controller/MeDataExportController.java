 ...
 *
 * <p>Returns JSON containing every record linked to the authenticated user: account info, follow
 * list, saved jobs, search history, interactions, HR affiliations.
 *
...
public class MeDataExportController {
  ...
  public ResponseEntity<Map<String, Object>> exportMyData(HttpServletRequest request) {
    ...
    var interactions =
        userJobInteractionRepository.findByAccountId(userId).stream()
            .map(
                i ->
                    Map.of(
                        "jobId", i.getJob() != null ? i.getJob().getId() : null,
                        "type",
                            i.getInteractionType() != null ? i.getInteractionType().name() : null,
                        "weight", i.getWeight()))
            .collect(Collectors.toList());
    ...
    var affiliations =
        companyHrAffiliationRepository.findByHrAccount_IdOrderByCreatedAtDesc(userId).stream()
            .map(
                a -> {
                  ...
                  m.put(
                      "submissionStatus",
                      a.getSubmissionStatus() != null ? a.getSubmissionStatus().name() : null);
    ...
    headers.setContentDisposition(
        org.springframework.http.ContentDisposition.attachment()
            .filename(
                "iting-data-export-" + account.getId() + "-" + System.currentTimeMillis() + ".json")
            .build());
