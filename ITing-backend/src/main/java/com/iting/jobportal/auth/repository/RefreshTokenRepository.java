...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, Long> {
  ...
  List<RefreshToken> findByUserIdAndIsUsedFalseAndIsRevokedFalseAndExpiryDateAfter(
  ...
  @Query(
      "SELECT COUNT(rt) FROM RefreshToken rt WHERE rt.userId = :userId AND rt.isUsed = false AND"
          + " rt.isRevoked = false AND rt.expiryDate > :now")
