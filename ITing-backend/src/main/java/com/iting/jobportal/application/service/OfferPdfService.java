 ...
 *
 * PDFBox built-in font Helvetica chỉ hỗ trợ Latin-1. Để render tiếng Việt
 * có dấu cần embed font Unicode (Noto Sans / DejaVu) — chưa có trong project,
 * nên tạm strip diacritics khi render. Caller có thể swap sang Unicode font
 * sau bằng cách load từ classpath: {@code PDType0Font.load(doc, stream)}.
 */
@Service
@Slf4j
public class OfferPdfService {
    ...
    public byte[] generate(OfferLetter offer, String candidateName, String companyName,
                           String companyAddress, String jobTitle) {
        try (PDDocument doc = new PDDocument(); ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            try (PDPageContentStream cs = new PDPageContentStream(doc, page)) {
                ...
                cs.setFont(PDType1Font.HELVETICA, 10);
                cs.newLineAtOffset(margin, y);
                cs.showText(strip("Ngay: " + offer.getSentAt().toLocalDate().format(D_FMT)
                ...
                cs.newLineAtOffset(margin, y);
                cs.showText(strip("Kinh gui Ong/Ba: " + (candidateName != null ? candidateName : "Ung vien")));
                cs.endText();
                y -= 22;

                // Body intro
                y = writeWrapped(cs, PDType1Font.HELVETICA, 11, margin, y, pageWidth - 2 * margin,
                        strip("Cong ty " + companyName + " tran trong gui den Ong/Ba thu moi nhan viec "
                ...
                if (offer.getSalaryAmount() != null) {
                    String salary = String.format("%,.0f %s / %s",
                            offer.getSalaryAmount(), offer.getSalaryCurrency(),
                ...
                y = writeKV(cs, "Han phan hoi:", offer.getExpiresAt().toLocalDate().format(D_FMT), margin, y);
                y -= 6;
                ...
                y -= 8;
                y = writeWrapped(cs, PDType1Font.HELVETICA, 11, margin, y, pageWidth - 2 * margin,
                        strip("Vui long phan hoi (Chap nhan / Tu choi) bang nut thao tac trong he thong ITing "
                                + "truoc " + offer.getExpiresAt().toLocalDate().format(D_FMT)
    ...
    private float writeKV(PDPageContentStream cs, String key, String value, float x, float y) throws IOException {
        cs.beginText();
        cs.setFont(PDType1Font.HELVETICA_BOLD, 11);
    ...
    private float writeWrapped(PDPageContentStream cs, PDType1Font font, float fontSize,
    ...
    private List<String> wrap(PDType1Font font, float fontSize, String text, float maxWidth) throws IOException {
