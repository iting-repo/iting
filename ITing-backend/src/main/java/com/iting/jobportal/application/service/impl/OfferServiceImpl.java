...
public class OfferServiceImpl implements OfferService {
    ...
    public OfferResponse create(Long hrAccountId, CreateOfferRequest req) {
        ...
        if (job.getCompany() == null || !hrCompanyId.equals(job.getCompany().getId())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Không có quyền tạo offer cho job này");
        }
        ...
        ApplyFormSentToJob app = applicationRepo.findById(pk)
        ...
        offerRepo.findFirstByApplyFormIdAndJobIdAndStatus(req.getApplyFormId(), req.getJobId(), OfferStatus.SENT)
                .ifPresent(existing -> {
                    throw new ResponseStatusException(HttpStatus.CONFLICT,
                            "Application này đã có offer đang chờ phản hồi (#" + existing.getId() + ")");
                });

        Account candidate = accountRepo.findById(app.getUserId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Không tìm thấy candidate account"));
        ...
        try {
            byte[] pdfBytes = offerPdfService.generate(
                    offer,
    ...
    private void notifyOfferSent(OfferLetter offer, Account candidate, Job job) {
        ...
        try {
            notificationService.createNotification(CreateNotificationRequest.builder()
                    ...
                    .content(String.format("%s đã gửi offer cho bạn cho vị trí \"%s\". Vui lòng phản hồi trước %s.",
    ...
    public OfferResponse revoke(Long hrAccountId, Long offerId) {
        OfferLetter offer = offerRepo.findById(offerId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Offer không tồn tại"));
        ...
        notifyCandidate(offer, NotificationType.OFFER_REVOKED,
    ...
    public OfferResponse acceptByCandidate(Long candidateAccountId, Long offerId) {
        ...
        applicationRepo.findById(pk).ifPresent(app -> {
        ...
        notifyHrOwner(offer, NotificationType.OFFER_ACCEPTED,
                "Ứng viên đã chấp nhận offer #" + offer.getId() + " cho vị trí \"" + offer.getPosition() + "\".");
    ...
    @Override
    @Transactional
    public OfferResponse declineByCandidate(Long candidateAccountId, Long offerId, DeclineOfferRequest body) {
        OfferLetter offer = requireCandidateOwnsOffer(offerId, candidateAccountId);
        requireOfferIsActive(offer);
        ...
        notifyHrOwner(offer, NotificationType.OFFER_DECLINED,
                "Ứng viên đã từ chối offer #" + offer.getId() + " cho vị trí \"" + offer.getPosition() + "\".");
    ...
    public List<OfferResponse> listByApplication(Long hrAccountId, Long applyFormId, Long jobId) {
        ...
        Job job = jobRepo.findById(jobId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Job không tồn tại"));
        if (job.getCompany() == null || !hrCompanyId.equals(job.getCompany().getId())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Không có quyền xem offer của application này");
        }
        return offerRepo.findByApplyFormIdAndJobIdOrderByCreatedAtDesc(applyFormId, jobId)
                .stream().map(OfferResponse::fromEntity).toList();
    }

    @Override
    @Transactional(readOnly = true)
    public List<OfferResponse> listMyOffers(Long candidateAccountId) {
        return offerRepo.findByCandidateAccountIdOrderByCreatedAtDesc(candidateAccountId)
                .stream().map(this::enrichForCandidate).toList();
    }
    ...
    public int expireOverdueOffers() {
        ...
        for (OfferLetter o : overdue) {
            ...
            // Notify cả 2 phía
            notifyCandidate(o, NotificationType.OFFER_EXPIRED,
                    "Offer cho vị trí \"" + o.getPosition() + "\" đã hết hạn.");
            notifyHrOwner(o, NotificationType.OFFER_EXPIRED,
                    "Offer #" + o.getId() + " (vị trí \"" + o.getPosition() + "\") đã hết hạn mà chưa được phản hồi.");
        }
    ...
    private void requireHrCanModify(OfferLetter offer, Long hrAccountId) {
        ...
        throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Chỉ HR chủ offer hoặc admin company mới được thao tác");
    }

    private void requireHrCanView(OfferLetter offer, Long hrAccountId) {
        Long hrCompanyId = authorizationService.requireApprovedCompanyOf(hrAccountId);
        if (!offer.getCompanyId().equals(hrCompanyId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Offer này không thuộc company của bạn");
        }
    }
    ...
    private OfferResponse enrichForCandidate(OfferLetter o) {
        ...
        jobRepo.findById(o.getJobId()).ifPresent(job -> {
