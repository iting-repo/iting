 ...
 *
 * <p>5-stage workflow: SCREENING → PHONE_SCREEN → INTERVIEW → OFFER → HIRED (+ REJECTED at any
 * point).
 *
 * <p>Move stage → records history + (optionally) sends templated email to candidate.
 *
 * <ul>
 *   <li>GET /api/hr/pipeline/stages — list valid stages
 *   <li>GET /api/hr/pipeline/jobs/{jobId} — applications grouped by stage
 *   <li>POST /api/hr/pipeline/applications/{applyFormId}/{jobId}/move — move stage + send email
 *   <li>GET /api/hr/pipeline/templates — email templates
 *   <li>POST /api/hr/pipeline/templates — create custom template
 * </ul>
 */
@RestController
...
public class HrPipelineController {
  ...
  public ResponseEntity<Map<String, List<Map<String, Object>>>> kanbanByJob(
    ...
    if (job.getCompany() == null || !hrCompanyId.equals(job.getCompany().getId())) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Không có quyền xem ứng viên job này");
    }
    ...
    for (Object[] row : rows) {
      ...
      if (row[1] != null) {
        accountRepository
            .findById(((Number) row[1]).longValue())
            .ifPresent(
                acc -> {
  ...
  public ResponseEntity<Map<String, Object>> moveStage(
    ...
    try {
      ...
      log.warn(
          "Failed to create in-app notification for stage move (applyFormId={}, toStage={})",
          applyFormId,
          toStage,
          e);
    }
  ...
  private void sendStageInAppNotification(ApplyFormSentToJob app, Job job, String toStage) {
    ...
    switch (toStage) {
      case "HIRED" -> {
        ...
        content =
            String.format(
                "%s đã chấp nhận hồ sơ của bạn cho vị trí \"%s\".", companyName, jobTitle);
      }
      case "REJECTED" -> {
        type = NotificationType.APPLICATION_REJECTED;
        content =
            String.format(
                "Đơn ứng tuyển của bạn cho vị trí \"%s\" tại %s đã bị từ chối.",
      ...
      default -> {
        ...
        content =
            String.format(
                "Hồ sơ của bạn cho vị trí \"%s\" tại %s đã chuyển sang giai đoạn %s.",
  ...
  private String sendStageEmail(
      ApplyFormSentToJob app, Job job, String toStage, Long templateId, Long hrId) {
    ...
    String subject =
        render(tpl.getSubject(), candidate.getFullName(), job.getTitle(), companyName, hrName);
    String body =
        render(tpl.getBody(), candidate.getFullName(), job.getTitle(), companyName, hrName);
  ...
  private String render(
      String template, String candidateName, String jobTitle, String companyName, String hrName) {
