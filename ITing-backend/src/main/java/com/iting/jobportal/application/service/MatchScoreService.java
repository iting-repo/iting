 ...
 *
 * <p>Luồng: lấy cvEmbedding (CV đang dùng của candidate) + jobEmbedding (Job) → gọi HF
 * /similarity/vectors → lưu match_score (0–100%) vào ApplyFormSentToJob.
 *
...
public class MatchScoreService {
  ...
  /**
   * Tính match score bất đồng bộ sau khi ứng viên apply. Nếu thiếu embedding → bỏ qua (match_score
   * = null).
   */
  @Async
  @Transactional
  public void computeAndSaveAsync(
      ApplyFormSentToJob.ApplyFormSentToJobId sentId, Long userId, Job job) {
    try {
      ...
      sentToJobRepository
          ...
          .ifPresent(
              sent -> {
                ...
                log.info(
                    "✅ Match score {}% saved for userId={} jobId={}",
                    String.format("%.1f", score), userId, job.getId());
              });
    } catch (Exception e) {
      log.warn(
          "Failed to compute match score for userId={} jobId={}: {}",
          userId,
          job.getId(),
          e.getMessage());
    }
