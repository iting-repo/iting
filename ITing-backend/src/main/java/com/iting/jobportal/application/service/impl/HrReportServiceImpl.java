...
/**
 * Tổng hợp số liệu cho dashboard báo cáo HR. Dùng native SQL cho query group-by hiệu năng tốt; mỗi
 * method là 1 query độc lập, kết quả ghép vào HrReportResponse cuối.
 */
@Service
...
public class HrReportServiceImpl implements HrReportService {
  ...
  private long countJobs(Long hrId, boolean activeOnly) {
    String sql =
        "SELECT COUNT(*) FROM job WHERE posted_by_hr_id = :hr"
...
""")
  ...
  private long countCreditTxBySource(
      Long accountId, String source, LocalDateTime from, LocalDateTime to) {
    Object o =
        em.createNativeQuery(
                """
  ...
  private long sumCreditAmount(
      Long accountId, boolean positiveOnly, LocalDateTime from, LocalDateTime to) {
  ...
  private long currentCreditBalance(Long accountId) {
    try {
      Object o =
          em.createNativeQuery("SELECT credits FROM account WHERE id = :acc")
              .setParameter("acc", accountId)
              .getSingleResult();
      return ((Number) o).longValue();
    } catch (Exception e) {
