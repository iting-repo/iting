...
public class CandidateApplicationServiceImpl implements CandidateApplicationService {
  ...
  public ApplicationSubmitResponse applyJob(Long userId, ApplyJobRequest request) {
    ...
    User user =
        userRepository
            .findById(userId)
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Không tìm thấy user"));
        ...
        appender ->
            appender.append(
                ...
                ApplicationCreatedEvent.of(
                    savedForm.getId(), request.getJobId(), userId, job.getPostedByHrId())));
    ...
    try {
      notificationService.createNotification(
          CreateNotificationRequest.builder()
              ...
              .content(
                  "Bạn đã ứng tuyển thành công vị trí \""
                      + job.getTitle()
                      + "\". Nhà tuyển dụng sẽ xem xét hồ sơ của bạn.")
    ...
    try {
      if (job.getPostedByHrId() != null) {
        String candidateName =
            user.getAccount() != null ? user.getAccount().getFullName() : "Ứng viên";
        notificationService.createNotification(
            CreateNotificationRequest.builder()
  ...
  public void withdrawApplication(Long userId, Long applicationId) {
    ...
    ApplyFormSentToJob sent =
        candidateApplicationRepository
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Không tìm thấy liên kết ứng tuyển"));
    ...
    if (sent.getStatus() == ApplicationStatus.REJECTED) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Đơn đã bị từ chối, không cần rút nữa.");
    }
  ...
  public Page<ApplicationResponse> getMyApplications(
    ...
    if (status != null && !status.isBlank() && !"ALL".equalsIgnoreCase(status)) {
      try {
        var statusEnum =
            com.iting.jobportal.application.entity.enums.ApplicationStatus.valueOf(
