...
public interface CandidateApplicationRepository
  ...
  @Query(
      "SELECT s FROM ApplyFormSentToJob s JOIN ApplyForm f ON f.id = s.id.applyFormId "
          + "WHERE f.userId = :userId AND s.status = :status")
  Page<ApplyFormSentToJob> findByUserIdAndStatus(
  ...
  @Query(
      "SELECT COUNT(s) > 0 FROM ApplyFormSentToJob s JOIN ApplyForm f ON f.id = s.id.applyFormId"
          + " WHERE f.userId = :userId AND s.id.jobId = :jobId AND s.status <>"
