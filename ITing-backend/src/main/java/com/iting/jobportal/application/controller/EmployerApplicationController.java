...
public class EmployerApplicationController {
    ...
    public ResponseEntity<Page<ApplicationResponse>> getApplicationsByJob(
        ...
        return ResponseEntity.ok(employerApplicationService.getApplicationsByJob(employerId, jobId, page, size));
    }

    @GetMapping
    @Operation(summary = "Tất cả đơn ứng tuyển của Employer")
    public ResponseEntity<Page<ApplicationResponse>> getAllApplications(
            @CurrentUser Long employerId,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(employerApplicationService.getAllApplicationsForEmployer(employerId, page, size));
    }
    ...
    public ResponseEntity<ApplicationResponse> updateStatus(
        ...
        return ResponseEntity.ok(employerApplicationService.updateApplicationStatus(employerId, id, request));
    }
    ...
    @GetMapping("/stats")
    @Operation(summary = "Thống kê tổng quan cho Employer")
