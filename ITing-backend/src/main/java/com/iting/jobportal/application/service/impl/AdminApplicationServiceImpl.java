...
public class AdminApplicationServiceImpl implements AdminApplicationService {
    ...
    public Page<ApplicationResponse> getApplicationsByJob(Long jobId, int page, int size) {
        ...
        return adminApplicationRepository.findByJobId(jobId, pageable)
                .map(sent -> {
                    ApplyForm form = applyFormRepository.findById(sent.getId().getApplyFormId())
                            .orElseThrow(() -> new RuntimeException("ApplyForm not found"));
                    return applicationMapperUtil.buildFullResponse(form, sent);
    ...
    public com.iting.jobportal.application.dto.response.JobApplicationStatsResponse
        ...
        for (Object[] r : rows) {
            ...
            if (status == null) { pending += count; continue; }
            switch (status) {
                case PENDING:   pending   += count; break;
                case VIEWED:    viewed    += count; break;
        ...
        double responseRate  = total   == 0 ? 0.0 : Math.round((employerResponded * 10000.0) / total) / 100.0;
