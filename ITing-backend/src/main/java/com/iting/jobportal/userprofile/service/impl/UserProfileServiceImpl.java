...
public class UserProfileServiceImpl implements UserProfileService {
  ...
  private UserProfile getProfileOrThrow(Long userId) {
    return userProfileRepo
        .findById(userId)
        .orElseThrow(
            () -> new ResourceNotFoundException("UserProfile not found with id: " + userId));
  }
            ...
            () -> {
              ...
              User user =
                  userRepository
                      ...
                      .orElseGet(
                          () -> {
                            com.iting.jobportal.auth.entity.Account account =
                                accountRepository
                                    .findById(userId)
                                    .orElseThrow(
                                        () ->
  ...
  public void updateSkill(Long id, SkillRequest req) {
    Skill skill =
        skillRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Skill not found"));
    skill.setName(req.getName());
  ...
  public void updateCVTitle(Long cvId, String title) {
    CV cv = cvRepo.findById(cvId).orElseThrow(() -> new ResourceNotFoundException("CV not found"));
    cv.setTitle(title);
    cvRepo.save(cv);
  }
