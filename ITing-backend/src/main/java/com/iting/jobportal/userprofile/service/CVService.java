...
public interface CVService {
  ...
  /** Upload a new CV for a user Automatically manages CV limit (max 3 CVs) */
  CVResponse uploadCV(Long userId, MultipartFile file, String title) throws IOException;
    ...
    /**
     * Trigger LLM scoring for a CV and persist the result.
     *
     * @param userId  the calling user's ID (for ownership check)
     * @param cvId    the CV to score
     * @param language output language — "vi" (default) for Vietnamese, "en" for English
     * @return the score response or null if CV not found / scoring failed
     */
    CvScoreResponse scoreCv(Long userId, Long cvId, String language);
}
