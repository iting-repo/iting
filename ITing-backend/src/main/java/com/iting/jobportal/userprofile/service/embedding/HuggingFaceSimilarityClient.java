 ...
 *
 * <ul>
 *   <li>{@code POST /similarity} — 2 text qua Gemma
 *   <li>{@code POST /similarity/minilm} — 2 text qua MiniLM
 *   <li>{@code POST /similarity/vectors} — 2 vector có sẵn (không cần model)
 * </ul>
 *
  ...
  private Optional<Double> post(String path, Map<String, Object> payload) {
    try {
      ...
      if (response.statusCode() / 100 != 2) {
        log.warn(
            "HF similarity {} failed: status={} body={}",
            path,
            response.statusCode(),
