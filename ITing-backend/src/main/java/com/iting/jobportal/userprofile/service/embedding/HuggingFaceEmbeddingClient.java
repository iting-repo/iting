...
/**
 * Embedding client tới Hugging Face AI Matching API (https://deeatetee-ai-matching-api.hf.space).
 *
 * <p>Endpoint chọn theo property {@code huggingface.ai.model}:
 *
 * <ul>
 *   <li>{@code gemma} (mặc định) → {@code POST /embed} — vector 768d cho CV/job matching.
 *   <li>{@code minilm} → {@code POST /embed/minilm} — vector 384d cho behavior matching.
 * </ul>
 *
 * Kích hoạt bằng {@code ai.provider=huggingface}. Khi đó bean này thay thế {@link
  ...
  public Optional<double[]> embed(String input) {
    ...
    try {
      ...
      if (response.statusCode() / 100 != 2) {
        log.warn(
            "HF embed failed: status={} body={}", response.statusCode(), truncate(response.body()));
