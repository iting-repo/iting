...
/**
 * Gọi {@code POST /extract-cv} của Hugging Face AI Matching API để parse CV (PDF/Image) thành JSON
 * cấu trúc + (tuỳ chọn) embedding vector.
 *
 * <p>Bean này luôn đăng ký (không phụ thuộc {@code ai.provider}) vì là tính năng standalone, không
 * thay thế cho EmbeddingClient chung.
 */
@Service
...
public class HuggingFaceCvExtractionClient {
  ...
  public record CvExtractionResult(
      String status, JsonNode extractedData, double[] embedding, String error) {
  ...
  public Optional<CvExtractionResult> extract(MultipartFile file) {
    ...
    try {
      ...
      HttpRequest.Builder builder =
          HttpRequest.newBuilder(URI.create(resolveEndpoint()))
      ...
      if (response.statusCode() / 100 != 2) {
        log.warn(
            "HF extract-cv failed: status={} body={}",
            response.statusCode(),
      ...
      return Optional.of(
          new CvExtractionResult(
              status, extracted.isMissingNode() ? null : extracted, embedding, error));
    } catch (Exception e) {
