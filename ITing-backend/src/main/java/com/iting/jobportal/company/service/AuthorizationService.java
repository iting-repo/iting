 ...
 *
 * <p>Thay thế cho pattern cũ {@code accountId == companyId} (do @MapsId). Mọi service cũ giả định
 * "current user id == company id" cần chuyển sang gọi {@link #requireApprovedCompanyOf(Long)} hoặc
 * {@link #requireCompanyOf(Long)}.
 */
@Service
...
public class AuthorizationService {
  ...
  /**
   * Trả về companyId mà HR được phép đăng job / cập nhật snapshot. Yêu cầu Affiliation.status =
   * APPROVED và Company.active = true.
   *
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.FORBIDDEN, "HR chưa được duyệt thuộc công ty nào"));
    if (!Boolean.TRUE.equals(aff.getCompany().getActive())) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Công ty đang bị tạm ngưng — không thể thực hiện thao tác này");
    }
    return aff.getCompany().getId();
  }

  /**
   * Trả về companyId của HR (kể cả khi affiliation đang INCOMPLETE/PENDING). Dùng cho xem profile,
   * sửa snapshot, upload license — tức các thao tác không yêu cầu HR đã hoàn tất xác minh.
   */
  public Long requireCompanyOf(Long hrAccountId) {
    return affiliationRepo
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(HttpStatus.FORBIDDEN, "HR chưa khởi tạo affiliation"));
  }

  /**
   * Trả về affiliation đang active của HR (entity, không chỉ id). Dùng cho mọi endpoint
   * /api/hr/affiliations/me/...
   */
