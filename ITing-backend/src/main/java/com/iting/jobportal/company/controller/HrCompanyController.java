 ...
 *
 * <p>Sau Phase 2/3, HR KHÔNG còn sửa Company info trực tiếp — mọi thay đổi đi qua affiliation
 * snapshot ({@code /api/hr/affiliations/me/...}). Controller này chỉ cung cấp endpoint read để HR
 * xem Company info hiện đang public.
 */
...
public class HrCompanyController {
  ...
  public ResponseEntity<CompanyResponse> getCompany(
      @Parameter(hidden = true) @CurrentUser Long hrAccountId, @PathVariable Long id) {
    ...
    if (!allowedCompanyId.equals(id)) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Bạn không có quyền xem thông tin công ty này");
    }
  ...
  public ResponseEntity<Map<String, String>> viewBusinessLicense(
    ...
    if (!allowedCompanyId.equals(id)) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Bạn không có quyền xem giấy phép của công ty này");
    }
  ...
  public ResponseEntity<Map<String, Long>> getFollowerCount(
    ...
    if (!allowedCompanyId.equals(id)) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Bạn không có quyền xem thông tin follower của công ty này");
    }
