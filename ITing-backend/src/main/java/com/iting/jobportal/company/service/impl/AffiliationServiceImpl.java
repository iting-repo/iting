...
public class AffiliationServiceImpl implements AffiliationService {
  ...
  public InitAffiliationResponse init(Long hrAccountId, InitAffiliationRequest request) {
    ...
    if (affiliationRepo.findActiveByHrAccountId(hrAccountId).isPresent()) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "HR đã có affiliation đang active — không thể init thêm");
    }

    Account hr =
        accountRepo
            .findById(hrAccountId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "Không tìm thấy HR account"));
    ...
    // 3) Tạo Affiliation INCOMPLETE — snapshot trống, chờ HR submit qua FoundingInfoTab +
    // Verification
  ...
  public Optional<AffiliationMeResponse> getMe(Long hrAccountId) {
    return affiliationRepo.findActiveByHrAccountId(hrAccountId).map(this::toMeResponse);
  }
  ...
  @Override
  @Transactional
  public AffiliationMeResponse updateBasicInfo(
      Long hrAccountId, UpdateAffiliationBasicInfoRequest req) {
    ...
    if (req.getIndustries() != null)
      aff.setSubmittedIndustriesJson(serializeIndustries(req.getIndustries()));
  ...
  public String uploadLogo(Long hrAccountId, MultipartFile file) {
    ...
    if (oldUrl != null && !oldUrl.isBlank()) {
      try {
        ...
        log.warn("Không xoá được logo cũ: {}", oldUrl, e);
      }
    }
    return newUrl;
  }
  ...
  public String uploadLicense(Long hrAccountId, MultipartFile file) {
    ...
    if (oldUrl != null && !oldUrl.isBlank()) {
      try {
        ...
        log.warn("Không xoá được license cũ: {}", oldUrl, e);
      }
    }
    return newUrl;
  }
  ...
  public String uploadConsent(Long hrAccountId, MultipartFile file, boolean confirmed) {
    ...
    if (oldUrl != null && !oldUrl.isBlank()) {
      try {
        ...
        log.warn("Không xoá được consent cũ: {}", oldUrl, e);
      }
    }
    return newUrl;
  }
  ...
  public AffiliationMeResponse submitReview(Long hrAccountId) {
    ...
    if (isBlank(aff.getSubmittedCompanyEmail()))
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Email công ty không được để trống");
  ...
  public String getLogoPresignedUrl(Long hrAccountId, int expiryMinutes) {
    ...
    if (isBlank(url)) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Chưa upload logo");
    return fileUploadService.generatePresignedUrl(url, expiryMinutes);
  }
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    ...
                    "HR chưa khởi tạo affiliation — vui lòng gọi POST /api/hr/affiliations/me/init"
                        + " trước"));
  }
