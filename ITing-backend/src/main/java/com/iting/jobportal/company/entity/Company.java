...
public class Company {
  ...
  /**
   * Affiliation đang là "nguồn" của info hiển thị hiện tại của Company. Set khi affiliation đầu
   * tiên APPROVED (auto-apply) hoặc admin gọi /api/admin/affiliations/{id}/apply-to-company sau
   * hotline call.
   */
  ...
  /**
   * JSON serialize của danh sách mạng xã hội: [{"platform":"FACEBOOK","url":"..."}]. Dùng JSON 1
   * cột thay vì bảng riêng vì dữ liệu nhỏ và frontend luôn replace-all. Đọc/ghi qua
   * CompanyService.getMySocialLinks / updateMySocialLinks (đã parse).
   */
