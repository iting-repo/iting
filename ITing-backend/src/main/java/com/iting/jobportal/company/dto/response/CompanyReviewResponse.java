...
public class CompanyReviewResponse {
    ...
    /**
     * Backward-compat: build response từ entity nhưng KHÔNG re-presign avatar.
     * Trả URL gốc trong DB — có thể đã hết hạn nếu là S3 presigned URL.
     * Caller nên dùng overload {@link #fromEntity(CompanyReview, FileUploadService)} thay vì gọi cái này.
     */
    ...
    /**
     * Build response từ entity và re-presign avatar URL (15 phút) để tránh lỗi
     * S3 "Request has expired" khi link được sinh ra cách đây > 1 giờ.
     *
     * @param review         entity nguồn
     * @param fileService    có thể null (skip re-presign). Truyền vào để re-presign.
     */
    public static CompanyReviewResponse fromEntity(CompanyReview review, FileUploadService fileService) {
    ...
    private static String resolveAvatar(String raw, FileUploadService fileService) {
        ...
        // Chỉ re-presign URL S3 (chứa amazonaws.com hoặc bắt đầu bằng s3 key). Để string bình thường yên.
