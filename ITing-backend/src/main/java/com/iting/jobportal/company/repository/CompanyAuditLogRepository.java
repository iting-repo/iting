...
public interface CompanyAuditLogRepository extends JpaRepository<CompanyAuditLog, Long> {
  ...
  @Query(
      "SELECT l FROM CompanyAuditLog l WHERE l.company.id = :companyId AND l.action = :action ORDER"
          + " BY l.createdAt DESC")
  List<CompanyAuditLog> findTopByCompanyIdAndActionOrderByCreatedAtDesc(
      @Param("companyId") Long companyId, @Param("action") CompanyAuditAction action);
}
