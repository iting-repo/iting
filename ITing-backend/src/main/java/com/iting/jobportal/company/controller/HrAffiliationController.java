...
/**
 * HR-side endpoints quản lý affiliation của HR đang login.
 * Mọi endpoint dưới /api/hr/affiliations/me/... đều require role EMPLOYER.
 */
...
public class HrAffiliationController {
    ...
    @PostMapping("/init")
    @Operation(summary = "Khởi tạo affiliation: tạo Company DRAFT (nếu taxCode chưa có) hoặc join Company existing")
    ...
    public ResponseEntity<AffiliationMeResponse> getMe(
            @Parameter(hidden = true) @CurrentUser Long hrAccountId) {
        return affiliationService.getMe(hrAccountId)
                .map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.noContent().build());
    }

    @PutMapping("/basic-info")
    @Operation(summary = "Cập nhật snapshot fields (name, description, website...). KHÔNG ghi vào Company.")
    ...
    public ResponseEntity<Map<String, String>> viewLicense(
            @Parameter(hidden = true) @CurrentUser Long hrAccountId) {
        return ResponseEntity.ok(Map.of("url", affiliationService.getLicensePresignedUrl(hrAccountId, 15)));
    }
