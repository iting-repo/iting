 ...
 *
 * Mỗi HR có 1 record active duy nhất (status IN INCOMPLETE/PENDING/APPROVED).
 * Một Company có thể có N affiliation (N HR cùng thuộc 1 công ty).
 *
 * Snapshot fields (submitted_*) lưu thông tin HR đã submit qua FoundingInfoTab.
 * Khi affiliation đầu tiên của Company được APPROVED → snapshot tự động được apply
 * lên Company (xem AdminAffiliationService.approve). Các affiliation sau APPROVED
 * không tự apply — phải qua endpoint /apply-to-company sau khi HR gọi tổng đài.
 */
@Entity
@Table(
