...
public class CompanyController {
  ...
  public CompanyController(
      ...
      org.springframework.beans.factory.ObjectProvider<RedisRateLimitingService>
          rateLimitProvider) {
  ...
  public ResponseEntity<Map<String, Object>> sendPhoneOtp(
    ...
    return ResponseEntity.ok(
        Map.of("message", "Đã gửi mã OTP. Vui lòng kiểm tra điện thoại.", "expiresInSeconds", 300));
  }
  ...
  public ResponseEntity<?> submitInfoReview(@Parameter(hidden = true) @CurrentUser Long userId) {

    if (redisRateLimitingService != null
        && !redisRateLimitingService.tryConsume(
            RateLimitPolicy.AI_REVIEW, String.valueOf(userId))) {
  ...
  public ResponseEntity<Map<String, String>> uploadLogo(
      @Parameter(hidden = true) @CurrentUser Long userId, @RequestPart("file") MultipartFile file) {
