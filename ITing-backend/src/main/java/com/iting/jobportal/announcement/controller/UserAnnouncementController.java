...
public class UserAnnouncementController {
  ...
  public ResponseEntity<List<AnnouncementDto>> getActive(
      @CurrentUser Long userId, @RequestParam(name = "route", defaultValue = "/") String route) {
    if (userId == null) return ResponseEntity.ok(List.of());
    String role =
        accountRepository.findById(userId).map(Account::getRole).map(Enum::name).orElse(null);
