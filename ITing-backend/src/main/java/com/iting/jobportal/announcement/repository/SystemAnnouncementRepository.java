...
public interface SystemAnnouncementRepository extends JpaRepository<SystemAnnouncement, Long> {
  ...
  /**
   * Lấy các announcement đang active & trong khoảng thời gian, sắp xếp priority desc. Lọc role +
   * route + ack ở tầng service (vì target_roles và trigger_routes là string).
   */
  @Query(
      """
      ...
      """)
  List<SystemAnnouncement> findActiveForUser(
