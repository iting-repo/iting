 ...
 *
 * <p>Note: same font limitation as SalaryReportPdfService — Helvetica strips Vietnamese
 * diacritics. Embed a TTF font for production.
 */
@Service
...
public class InvoicePdfService {
    ...
    /**
     * Auto-generate an invoice for a paid order if not already invoiced.
     * Called from SepayPaymentService.activatePurchase().
     */
    @Transactional
    public Invoice autoGenerateForPaidOrder(PaymentOrder order) {
        ...
        Invoice invoice = Invoice.builder()
                ...
                .vatAmount(vat)
                .totalAmount(total)
                .itemDescription(order.getDescription() != null ? order.getDescription() : order.getItemType())
                .build();

        // Render PDF + upload S3
        byte[] pdfBytes = renderPdf(invoice);
        String s3Url = fileUploadService.uploadBytes(
                pdfBytes,
        ...
        log.info("[Invoice] Generated {} for order {} (account {})",
                invoice.getInvoiceNumber(), order.getOrderCode(), acc.getId());
        return invoice;
    }
    ...
    public String nextInvoiceNumber() {
        ...
        InvoiceSequence seq = sequenceRepository.findByYearForUpdate(year)
                .orElseGet(() -> {
    ...
    private byte[] renderPdf(Invoice inv) {
        try (PDDocument doc = new PDDocument();
            ...
            try (PDPageContentStream c = new PDPageContentStream(doc, page)) {
                ...
                drawText(c, inv.getIssuedAt().format(DateTimeFormatter.ofPattern("dd MMM yyyy")),
                        220, 705, FONT_REGULAR, 12, NAVY);

                drawText(c, "Order Code", 400, 720, FONT_BOLD, 10, NAVY);
                drawText(c, inv.getPaymentOrder() != null ? inv.getPaymentOrder().getOrderCode() : "-",
                        400, 705, FONT_REGULAR, 12, NAVY);
                ...
                drawText(c, "Tax code (MST): " + nullSafe(inv.getBillToTaxCode()), 50, 612, FONT_REGULAR, 11, NAVY);
                drawText(c, "Address: " + nullSafe(inv.getBillToAddress()), 50, 596, FONT_REGULAR, 11, NAVY);
                ...
                drawText(c, "Transaction ID: " +
                                (inv.getPaymentOrder() != null && inv.getPaymentOrder().getSepayTransactionId() != null
                                        ? inv.getPaymentOrder().getSepayTransactionId() : "-"),
                        40, 390, FONT_REGULAR, 10, NAVY);

                // Footer
                drawText(c, "This is an electronic invoice — no signature required.",
                        40, 80, FONT_REGULAR, 9, new Color(120, 120, 120));
                drawText(c, "ITING JSC  |  https://iting.vn  |  support@iting.vn",
                        40, 65, FONT_REGULAR, 9, new Color(120, 120, 120));
                fillRect(c, 0, 0, 595, 30, NAVY);
                drawText(c, "Generated by ITing — " +
                        LocalDate.now().format(DateTimeFormatter.ISO_DATE),
                        40, 12, FONT_REGULAR, 9, Color.WHITE);
            }
            doc.save(out);
    ...
    private void fillRect(PDPageContentStream c, float x, float y, float w, float h, Color color) throws Exception {
