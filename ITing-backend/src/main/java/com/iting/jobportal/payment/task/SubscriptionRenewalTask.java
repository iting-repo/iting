 ...
 *
 * <p>Auto-renew flow (NOT auto-charge yet — SEPAY doesn't support card-on-file; bank-transfer
 * requires manual action). Instead: 1. T-3 days: send "Your subscription expires in 3 days — pay to
 * renew" email with QR link. 2. T-0: mark status=EXPIRED and downgrade HR to free tier. 3. T+7:
 * send "We miss you" win-back email if still expired.
 */
@Component
...
public class SubscriptionRenewalTask {
  ...
  public void processExpiringSubscriptions() {
    ...
    List<HrSubscription> expired =
        subscriptionRepository.findByStatusAndAutoRenewAndExpiresAtBefore("ACTIVE", true, now);
    for (HrSubscription sub : expired) {
      sub.setStatus("EXPIRED");
      subscriptionRepository.save(sub);
      log.info(
          "[Renewal] Marked subscription {} EXPIRED for account {}",
          sub.getId(),
    ...
    List<HrSubscription> expiringSoon =
        subscriptionRepository.findByStatusAndAutoRenewAndExpiresAtBefore(
            "ACTIVE", true, threeDaysAhead);
