...
public class CreditServiceImpl implements CreditService {
    ...
    @Override
    @Transactional
    public CreditTransaction grant(Long accountId, int amount, String source, Long referenceId, String description) {
        if (amount <= 0) {
        ...
        log.info("[CREDIT] grant account={} amount={} source={} balance {}→{}",
                accountId, amount, source, current, next);

        return txRepository.save(CreditTransaction.builder()
                .account(account)
                .amount(amount)
                .balanceAfter(next)
                .source(source)
    ...
    @Override
    @Transactional
    public CreditTransaction consume(Long accountId, int amount, String source, Long referenceId, String description) {
        if (amount <= 0) {
        ...
        log.info("[CREDIT] consume account={} amount={} source={} balance {}→{}",
                accountId, amount, source, current, next);
