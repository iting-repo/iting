...
/**
 * Writes a domain event to the outbox table inside the caller's DB transaction.
 * If the surrounding transaction rolls back, the event is rolled back too — guaranteeing
 * "no event without state change, no state change without event".
 *
 * Only active when Kafka is enabled. Callers should inject {@code Optional<OutboxAppender>}.
 */
@Service
