...
public class DistributedLockService {
    ...
    /**
     * Run {@code action} only if the lock is acquired within {@code waitMs}, holding it for at most {@code leaseMs}.
     * Throws {@link LockAcquisitionException} on failure to acquire — callers decide retry/fail-fast.
     */
    ...
    public void withLock(String key, long waitMs, long leaseMs, Runnable action) {
        withLock(key, waitMs, leaseMs, () -> { action.run(); return null; });
    }
