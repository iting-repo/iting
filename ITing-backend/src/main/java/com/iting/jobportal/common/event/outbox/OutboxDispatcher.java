...
/**
 * Polls outbox table and ships PENDING rows to Kafka. Uses {@code FOR UPDATE SKIP LOCKED}
 * so multiple replicas can run safely. Enable only on the {@code worker} profile in production
 * to avoid every API instance polling the DB.
 */
@Component
        ...
        for (OutboxEvent row : batch) {
            try {
                ...
                if (row.getAttempts() >= 10) {
                    ...
                    log.error("Outbox row {} marked FAILED after {} attempts: {}",
