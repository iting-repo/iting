...
/**
 * Direct Kafka publisher. Use this for fire-and-forget signals.
 * For events that MUST not be lost on rollback, route through the Outbox instead.
 */
@Service
...
public class DomainEventPublisher {
    ...
    private CompletableFuture<SendResult<String, Object>> send(String topic, String key, Object payload) {
        return kafkaTemplate.send(topic, key, payload).whenComplete((res, ex) -> {
            if (ex != null) {
                log.error("Kafka publish failed topic={} key={} : {}", topic, key, ex.getMessage());
            } else {
                log.debug("Published topic={} key={} offset={}", topic, key, res.getRecordMetadata().offset());
            }
        });
    }
}
