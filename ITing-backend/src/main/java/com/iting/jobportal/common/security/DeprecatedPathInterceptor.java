...
/**
 * Log warning + thêm response header {@code X-Deprecated-Path: replacement} cho mọi
 * URL legacy đã có alias mới sau Phase 4.
 *
 * Mục đích: monitor xem FE/client nào còn gọi alias cũ để track lúc nào đủ điều
 * kiện remove old controllers.
 *
 * Lifecycle: dual-mount 2 sprint, sau đó remove cả interceptor lẫn old controllers.
 */
@Slf4j
@Component
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        for (Map.Entry<String, String> e : DEPRECATIONS.entrySet()) {
            if (uri.startsWith(e.getKey())) {
                ...
                log.warn("[DEPRECATED] {} {} → please migrate to {} (will be removed in 2 sprints)",
                        request.getMethod(), uri, replacement);
