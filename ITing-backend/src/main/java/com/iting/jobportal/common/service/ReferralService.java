...
public class ReferralService {
  ...
  public ReferralCode getOrCreateCodeFor(Account account) {
    ...
    ReferralCode rc =
        ReferralCode.builder().account(account).code(code).totalInvited(0).totalSignups(0).build();
    return codeRepository.save(rc);
  }

  /**
   * Khi 1 user mới đăng ký với `?ref=CODE`, attribute họ vào referrer's account. Idempotent — đã có
   * Referral cho `referred` thì return existing.
   */
  @Transactional
  public Optional<Referral> attributeSignup(String refCode, Account newAccount) {
    ...
    log.info(
        "Referral attributed: referrer={} referred={}",
  ...
  /**
   * Đánh dấu conversion khi referred user submit ứng tuyển đầu tiên. Trigger: gọi từ
   * CandidateApplicationServiceImpl.applyJob.
   */
  @Transactional
  public void markFirstApplication(Long referredAccountId) {
    referralRepository
        .findByReferred_Id(referredAccountId)
