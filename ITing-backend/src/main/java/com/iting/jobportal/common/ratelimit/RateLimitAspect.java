...
public class RateLimitAspect {
    ...
    @Autowired(required = false) private RedisRateLimitingService redisLimiter;
    @Autowired(required = false) private InMemoryRateLimiter localLimiter;
    ...
    private String resolveSubject(String spec, ProceedingJoinPoint pjp, MethodSignature sig) {
        ...
        if ("user".equalsIgnoreCase(spec)) return Optional.ofNullable(currentUserId()).orElse(clientIp());
