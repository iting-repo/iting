...
/**
 * Tự động ghi log mỗi hành động ADMIN có side-effect (POST/PUT/DELETE/PATCH
 * trên bất kỳ controller nào trong gói admin.controller). Trang
 * /admin/audit từng rỗng vì service.logActivity() chưa được gọi từ đâu cả;
 * aspect này giải quyết bằng cách "logging-as-a-cross-cutting-concern" thay
 * vì rải save() khắp từng endpoint.
 *
 * Chiến lược:
 *   • Chỉ log SAU KHI handler trả về thành công (@AfterReturning) — request
 *     fail (4xx/5xx) không tạo nhiễu trong audit.
 *   • Bỏ GET — chỉ những thao tác thay đổi state mới quan trọng để audit.
 *   • Lỗi khi ghi log KHÔNG được throw — nuốt với log.warn để không vỡ
 *     business flow vì hệ thống audit "best-effort".
 */
@Aspect
@Component
