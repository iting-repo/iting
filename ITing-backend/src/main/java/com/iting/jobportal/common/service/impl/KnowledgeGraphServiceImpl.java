...
/**
 * In-memory Knowledge Graph implementation. Loads skill ontology and synonyms from JSON files at
 * startup. Uses adjacency list for graph traversal (BFS expansion).
 */
@Service
...
public class KnowledgeGraphServiceImpl implements KnowledgeGraphService {
  ...
  public void init() {
    ...
    loadLocationGraph();
    log.info(
        "✅ Knowledge Graph loaded: {} nodes, {} edges, {} synonyms, {} province→region mappings",
        nodeLabels.size(),
                ...
                outerEntry -> {
                  ...
                  outerEntry
                      ...
                      .forEachRemaining(
                          innerEntry ->
                              innerMap.put(
                                  innerEntry.getKey(), innerEntry.getValue().asDouble(0.0)));
  ...
  public List<String> explainMatch(List<String> cvSkills, List<String> jdSkills) {
    ...
    for (String cvSkill : cvSkills) {
      ...
      for (String jdSkill : jdSkills) {
        ...
        // Direct match
        if (cvId.equals(jdId)) {
          explanations.add(String.format("✅ \"%s\" khớp trực tiếp với \"%s\"", cvSkill, jdSkill));
          continue;
        }
        ...
        if (path != null && !path.isEmpty()) {
          String pathStr = path.stream().map(this::getLabel).collect(Collectors.joining(" → "));
          explanations.add(String.format("✅ \"%s\" → %s → \"%s\"", cvSkill, pathStr, jdSkill));
        }
      }
    }
