 ...
 *
 * <p>Conversion = referred user submit ứng tuyển đầu tiên (firstApplicationAt IS NOT NULL).
 * Mỗi 5 conversion (chưa rewarded) → cấp Premium 30 ngày + gửi email thông báo.
 *
...
public class ReferralRewardTask {
    ...
    public void grantRewards() {
        ...
        List<Referral> pending = referralRepository.findAll().stream()
        ...
        for (Map.Entry<Long, java.util.List<Referral>> entry : byReferrer.entrySet()) {
            try {
                ...
                log.error("Failed to grant referral rewards for referrer {}: {}",
    ...
    private void processReferrerBatch(Long referrerId, List<Referral> conversions) {
        ...
        List<Referral> toReward = conversions.stream()
        ...
        LocalDateTime baseline = currentExpiry != null && currentExpiry.isAfter(now)
                ? currentExpiry : now;
        ...
        log.info("Referral reward: referrer={} batches={} (×{} days) → premium until {}",
                referrerId, batches, PREMIUM_DAYS, newExpiry);
    ...
    private void sendRewardEmail(Account referrer, int batches) {
        ...
        String body = "Chào " + (referrer.getFullName() != null ? referrer.getFullName() : "bạn") + ",\n\n"
            + "Chúc mừng! Bạn đã giới thiệu thành công " + (batches * CONVERSIONS_PER_REWARD)
            + " ứng viên mới đã apply việc qua link của bạn.\n\n"
            + "🎁 Phần thưởng: " + days + " NGÀY PREMIUM được cộng vào tài khoản.\n"
            + "Premium expiry: " + referrer.getPremiumUntil() + "\n\n"
            + "Quyền lợi Premium:\n"
