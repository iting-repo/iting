 ...
 * <ol>
 *   <li>Cover page
 *   <li>Executive summary (total jobs, avg salary, key insights)
 *   <li>Salary by experience level (Junior/Mid/Senior...)
 *   <li>Salary by job type (full-time, part-time, contract)
 *   <li>Top locations
 *   <li>Methodology + footer
 * </ol>
 *
 * <p><strong>Font caveat:</strong> Uses Helvetica (Type1) which lacks Vietnamese diacritics
 * support. Text uses ASCII / English. For Vietnamese text rendering, embed a TTF font like <code>
 * Roboto-Regular.ttf</code> via <code>PDType0Font.load(doc, ttfStream)</code>.
 */
@Service
...
public class SalaryReportPdfService {
   ...
   *
   * @return the public S3 URL (use {@link FileUploadService#generatePresignedUrl} to get a signed
   *     URL).
   */
  public String generateAndUpload() {
    ...
    log.info(
        "[SalaryReport] PDF generated: {} bytes — uploading to S3 key={}", pdfBytes.length, S3_KEY);
  ...
  /**
   * Return cached/static URL if configured, otherwise the canonical S3 public URL pattern. Combined
   * with {@link FileUploadService#generatePresignedUrl} to deliver a signed download.
   */
  ...
  private ReportData buildReportData(List<Job> jobs) {
    ...
    Map<ExperienceLevel, List<Job>> byExp =
        jobs.stream()
            .filter(j -> j.getExperienceLevel() != null)
            .collect(Collectors.groupingBy(Job::getExperienceLevel));
    d.byExperience =
        byExp.entrySet().stream()
            .map(
                e ->
                    new Bucket(
                        e.getKey().name(), e.getValue().size(), median(salaryList(e.getValue()))))
  ...
  private void renderCoverPage(PDDocument doc) throws Exception {
    ...
    try (PDPageContentStream c = new PDPageContentStream(doc, page)) {
      ...
      drawText(
          c, "Generated from real job postings on iting.vn", 40, 460, FONT_REGULAR, 12, BRAND_GRAY);
  ...
  private void renderMethodologyPage(PDDocument doc, ReportData data) throws Exception {
    ...
    try (PDPageContentStream c = new PDPageContentStream(doc, page)) {
      ...
      drawText(
          c,
          "Real active job postings on iting.vn at the time of report generation.",
          40,
          700,
          FONT_REGULAR,
          11,
      ...
      drawText(
          c,
          "Figures reflect job postings, not negotiated offers. Actual",
          40,
          460,
          FONT_REGULAR,
          11,
      ...
      drawText(
          c,
          "compensation may vary based on individual negotiations,",
          40,
          445,
          FONT_REGULAR,
          11,
      ...
      drawText(
          c,
          "benefits, equity, and bonuses not captured in this dataset.",
          40,
          430,
          FONT_REGULAR,
          11,
  ...
  private void statBox(
      PDPageContentStream c, int x, int y, int w, int h, String label, String value)
      throws Exception {
  ...
  private void renderBarChart(PDPageContentStream c, List<Bucket> buckets, int x, int y)
      throws Exception {
    if (buckets.isEmpty()) {
  ...
  private void tableHeader(PDPageContentStream c, String col1, String col2, String col3)
      throws Exception {
  ...
  private void tableRow(PDPageContentStream c, String label, int count, String median, int y)
      throws Exception {
  ...
  private List<String> buildInsights(ReportData d) {
    ...
    if (d.totalJobs > 0) {
      out.add(
          "Median monthly salary across " + d.totalJobs + " jobs: " + formatVnd(d.medianMonthly));
    }
