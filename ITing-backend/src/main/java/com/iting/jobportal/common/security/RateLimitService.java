...
public class RateLimitService {
    ...
    public Bucket resolveRefreshBucket(String ip) {
        return buckets.computeIfAbsent("refresh-" + ip, k -> Bucket.builder()
                .addLimit(Bandwidth.classic(10, Refill.intervally(10, Duration.ofMinutes(1))))
                .build());
    }
    ...
    public Bucket resolveSearchBucket(String ip) {
        return buckets.computeIfAbsent("search-" + ip, k -> Bucket.builder()
    ...
    public Bucket resolveAdminBucket(String ip) {
        return buckets.computeIfAbsent("admin-" + ip, k -> Bucket.builder()
                .addLimit(Bandwidth.classic(20, Refill.intervally(20, Duration.ofMinutes(1))))
                .build());
    }
}
