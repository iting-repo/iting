...
public class NotificationServiceImpl implements NotificationService {
  ...
  public NotificationResponse createNotification(CreateNotificationRequest request) {
    ...
    if (pushAllowed) {
      webSocketNotificationService.sendNotificationToRecipient(
          saved.getRecipientId(), saved.getRecipientType(), response);
    }
    // Unread badge vẫn cập nhật (không phá UX — user vẫn cần biết có thông báo mới khi mở app)
    webSocketNotificationService.sendUnreadCount(
        saved.getRecipientId(), saved.getRecipientType(), unreadCount);

    // Gửi Mail thông báo — chỉ khi emailEnabled và không trong quiet hours
    if (emailAllowed) {
      try {
        accountRepository
            .findById(saved.getRecipientId())
            .ifPresent(
                account -> {
                  ...
                  String actionUrl =
                      "http://localhost:3000"
                          + (saved.getActionUrl() != null ? saved.getActionUrl() : "");
                    ...
                    try {
                      ApplyFormSentToJob sent =
                          employerApplicationRepository
                              .findByIdApplyFormId(saved.getEntityId())
                              .orElseThrow(
                                  () -> new RuntimeException("ApplyFormSentToJob not found"));
                      Job job =
                          jobRepository
                      ...
                      if (saved.getType() == NotificationType.APPLICATION_ACCEPTED) {
                        ...
                        htmlContent =
                            emailTemplateService.getApplicationAcceptedTemplate(
                        ...
                        htmlContent =
                            emailTemplateService.getApplicationRejectedTemplate(
                                (user.getAccount() != null
                                    ? user.getAccount().getFullName()
                                    : null),
                                job.getTitle(),
                      ...
                      log.warn(
                          "Could not load details for application template, falling back to"
                              + " generic: {}",
                          e.getMessage());
                    }
                  }
  ...
  /**
   * Map NotificationType → flag getter trên NotificationPreference. Trả về true nếu user cho phép
   * category, false nếu tắt. Các loại không map (admin-only) → mặc định true để không miss noti hệ
   * thống.
   */
  private boolean isCategoryEnabled(NotificationPreference pref, NotificationType type) {
    Function<NotificationPreference, Boolean> getter =
        switch (type) {
          ...
          case MESSAGE_NEW -> NotificationPreference::getNewMessages;
          case COMPANY_UPDATE, COMPANY_NEW_JOB -> NotificationPreference::getFollowedCompanies;
          case SYSTEM_ANNOUNCEMENT,
            ...
            // Admin-only / Company-only types → không enforce preference (USER không nhận các loại
            // này)
  ...
  /**
   * Tạo response "blocked" để caller không NPE. id = null cho biết noti không được tạo. Caller có
   * thể check `response.getId() == null` nếu cần phân biệt.
   */
  ...
  public Page<NotificationResponse> getNotifications(
    ...
    return notificationRepository
        .findByRecipientIdAndRecipientType(recipientId, recipientType, pageable)
        .map(this::toResponse);
  }
  ...
  public List<NotificationResponse> getUnreadNotifications(
    ...
    return notificationRepository
        .findUnreadByRecipientIdAndRecipientType(recipientId, recipientType)
        .stream()
        .map(this::toResponse)
  ...
  public Long getUnreadCount(Long recipientId, RecipientType recipientType) {
    return notificationRepository.countUnreadByRecipientIdAndRecipientType(
  ...
  public void markAllAsRead(Long recipientId, RecipientType recipientType) {
    notificationRepository.markAllAsReadForRecipient(
        recipientId, recipientType, LocalDateTime.now());
  ...
  public void deleteNotification(
    ...
    Long unreadCount =
        notificationRepository.countUnreadByRecipientIdAndRecipientType(recipientId, recipientType);
    webSocketNotificationService.sendUnreadCount(recipientId, recipientType, unreadCount);
  }
