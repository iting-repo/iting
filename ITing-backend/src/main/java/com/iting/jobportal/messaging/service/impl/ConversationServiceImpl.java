...
public class ConversationServiceImpl implements ConversationService {
  ...
  public ConversationListResponse getConversationsForUser(Long userId, int page, int size) {
    ...
    Pageable pageable =
        PageRequest.of(safePage, safeSize, Sort.by(Sort.Direction.DESC, "lastMessageTime"));
  ...
  @Override
  public ConversationListResponse getConversationsByType(
      Long userId, ConversationType type, int page, int size) {
    int safePage = Math.max(page, 0);
    int safeSize = (size <= 0 || size > 100) ? 20 : size;
    ...
    Page<Conversation> conversationPage =
        conversationRepository.findByUserIdAndType(userId, type, pageable);
  ...
  private ConversationResponse convertToConversationResponse(
    ...
    Long unreadCount =
        messageRepository.countUnreadByConversationIdAndReceiverId(
    ...
    Long otherParticipantId =
        conversation.getParticipant1Id().equals(currentUserId)
  ...
  private void fillParticipantProfile(
      ConversationResponse response,
      Long participantId,
    ...
    if (company != null) {
      ...
      if (!participantId.equals(currentUserId)) {
        response.setOtherParticipantActive(
            company.getActive() != null ? company.getActive() : true);
      }
    }
  }
}
