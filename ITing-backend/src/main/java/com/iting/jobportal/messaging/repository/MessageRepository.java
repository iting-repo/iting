...
public interface MessageRepository extends JpaRepository<Message, Long> {
  ...
  @Query(
      "SELECT m FROM Message m WHERE m.conversationId = :conversationId ORDER BY m.createdAt ASC")
  ...
  @Query(
      "SELECT COUNT(m) FROM Message m WHERE m.conversationId = :conversationId "
          + "AND m.receiverId = :receiverId AND m.isRead = false")
  Long countUnreadByConversationIdAndReceiverId(
      @Param("conversationId") Long conversationId, @Param("receiverId") Long receiverId);
  ...
  @Query(
      "SELECT m FROM Message m WHERE m.receiverId = :userId AND m.isRead = false "
  ...
  @Modifying
  @Query(
      "UPDATE Message m SET m.isRead = true, m.readAt = :readAt WHERE m.conversationId ="
          + " :conversationId AND m.receiverId = :receiverId AND m.isRead = false")
