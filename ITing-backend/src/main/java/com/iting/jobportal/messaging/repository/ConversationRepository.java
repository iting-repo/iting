...
public interface ConversationRepository extends JpaRepository<Conversation, Long> {
  ...
  /**
   * Find conversation between two participants (order-independent) Works for both USER_USER and
   * USER_COMPANY conversations
   */
  @Query(
  ...
  @Query(
      "SELECT c FROM Conversation c WHERE c.type = :type AND "
          + "((c.participant1Id = :userId1 AND c.participant2Id = :userId2) OR "
          + "(c.participant1Id = :userId2 AND c.participant2Id = :userId1))")
  ...
  Page<Conversation> findByUserIdAndType(
      @Param("userId") Long userId, @Param("type") ConversationType type, Pageable pageable);
  ...
  @Query(
      "SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Conversation c WHERE "
          + "(c.participant1Id = :userId1 AND c.participant2Id = :userId2) OR "
          + "(c.participant1Id = :userId2 AND c.participant2Id = :userId1)")
