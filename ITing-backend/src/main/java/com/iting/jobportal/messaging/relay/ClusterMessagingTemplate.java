 ...
 *
 * Behavior:
 *   {@link RelayPublisher} present  → publishes to Redis; every instance (including this one) consumes and dispatches locally
 *   absent                          → falls back to local SimpMessagingTemplate (single-instance mode)
 *
 * The publisher is only created when {@code app.cluster.messaging.enabled=true} AND a
 * {@link RedisTemplate} bean exists (i.e. {@code app.redis.enabled=true}).
 */
@Component
...
public class ClusterMessagingTemplate {
    ...
    public void convertAndSendToUser(String userId, String destination, Object payload) {
        if (relay.isPresent()) {
            relay.get().publish(WsBroadcastEnvelope.toUser(relay.get().originId(), userId, destination, payload));
        } else {
    ...
    public void convertAndSend(String destination, Object payload) {
        if (relay.isPresent()) {
            relay.get().publish(WsBroadcastEnvelope.toTopic(relay.get().originId(), destination, payload));
        } else {
