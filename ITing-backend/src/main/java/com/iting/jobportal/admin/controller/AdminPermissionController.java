...
public class AdminPermissionController {
    ...
    @PostMapping("/bulk-overrides")
    @Operation(summary = "Áp dụng cùng một bộ permission override cho nhiều người dùng cùng lúc")
    public ResponseEntity<?> bulkReplaceOverrides(@RequestBody BulkOverrideRequest request) {
        Long adminId = 1L;
        adminPermissionService.bulkReplaceOverrides(adminId, request.getUserIds(), request.getOverrides());
        return ResponseEntity.ok(Map.of(
            "message", "Bulk overrides applied successfully",
