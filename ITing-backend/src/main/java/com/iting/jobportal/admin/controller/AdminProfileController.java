...
/**
 * Hồ sơ nhân viên admin: thông tin kế thừa từ Account (email, status, fullName,
 * v.v.) + staffCode/adminLevel riêng. Tự động khởi tạo Admin row + staffCode
 * lần đầu admin gọi (không cần seed script).
 */
@RestController
...
public class AdminProfileController {
    ...
    @Transactional
    public ResponseEntity<AdminProfileResponse> getMyProfile(@AuthenticationPrincipal AuthUser principal) {
        if (principal == null) {
        ...
        Admin admin = adminRepository.findById(account.getId())
                .orElseGet(() -> provisionAdminRow(account));

        return ResponseEntity.ok(AdminProfileResponse.from(account, admin));
    }

    /**
     * Tạo Admin row lần đầu cho 1 Account đã có role=ADMIN nhưng chưa có
     * record trong admin_accounts. staffCode generate theo format
     * ITN-{id padded 4 chữ số} — deterministic, dễ đọc, không trùng vì
