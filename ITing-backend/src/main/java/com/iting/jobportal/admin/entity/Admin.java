 ...
 * status) lives on Account. Entity này chỉ giữ field riêng cho admin:
 * staff_code (mã định danh nhân viên ITing) + admin_level (cấp bậc).
 *
 * <p>NOTE: cờ "active" và "notes" trong revision trước đã được drop bởi V83 và
 * không bao giờ được tạo lại — bỏ luôn để entity match DB thực tế. Suspend
 * admin dùng Account.status thay vì cờ riêng.
 */
@Entity
