...
public class AdminBlogServiceImpl implements AdminBlogService {
    ...
    /**
     * Sorting rule (automatic):
     * 1. is_featured = true → always on top
     * 2. Within same group (featured / normal) → newest first (created_at DESC)
     */
    @Override
    ...
    public Page<Blog> getBlogs(String keyword, String status, int page, int size) {
        ...
        String st = (status != null && !status.isBlank() && !"all".equalsIgnoreCase(status)) ? status.toUpperCase() : null;
    ...
    public Blog updateBlog(Long id, BlogRequest request) {
        ...
        blog.setStatus(request.getStatus() != null ? request.getStatus().toUpperCase() : blog.getStatus());
        blog.setSummary(request.getSummary());
        blog.setContent(request.getContent());
        blog.setThumbnailUrl(request.getThumbnailUrl());
        blog.setAuthor(request.getAuthor());
        blog.setIsFeatured(request.getIsFeatured() != null ? request.getIsFeatured() : blog.getIsFeatured());
     ...
     * "Xu hướng tuyển dụng IT 2024" → "xu-huong-tuyen-dung-it-2024"
     */
