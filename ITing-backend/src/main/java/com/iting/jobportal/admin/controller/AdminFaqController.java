...
public class AdminFaqController {
  ...
  public ResponseEntity<Page<StaticContent>> list(
    ...
    Pageable pageable =
        PageRequest.of(
            page, size, Sort.by(Sort.Order.asc("sortOrder"), Sort.Order.desc("createdAt")));
    ...
    if (hasKeyword && published != null) {
      result =
          contentRepository.findByTypeAndPublishedAndTitleContainingIgnoreCase(
              FAQ_TYPE, published, keyword.trim(), pageable);
    } else if (hasKeyword) {
      result =
          contentRepository.findByTypeAndTitleContainingIgnoreCase(
  ...
  /**
   * Resolve slug: nếu user nhập → dùng (đã chuẩn hoá). Nếu không → sinh từ title. Đảm bảo unique
   * bằng cách thêm hậu tố nếu trùng (trừ trường hợp chính nó).
   */
  private String resolveSlug(String inputSlug, String title, String currentSlug) {
    ...
    int suffix = 1;
    while (contentRepository.existsBySlug(candidate) && !candidate.equals(currentSlug)) {
