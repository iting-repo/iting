...
public interface UserJobInteractionRepository extends JpaRepository<UserJobInteraction, Long> {
     ...
     * Job mà user tương đồng đã APPLY (weight=5) sẽ xếp cao hơn job chỉ được VIEW (weight=1).
     */
    @Query(value = """
     ...
     * Dùng để xây dựng "behavioral preference profile" — suy ra user thích skill/location nào.
     */
