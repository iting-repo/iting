...
public class RecommendationServiceImpl implements RecommendationService {
     ...
     * 
     * Cách hoạt động:
     * - Lấy 20 interaction gần nhất (ưu tiên weight cao: APPLY > SAVE > CLICK >
     * VIEW)
     * - Trích xuất skills và locations từ các job đó
     * - Tính trọng số: skill từ job đã APPLY (weight=5) mạnh hơn skill từ job đã
     * VIEW (weight=1)
     * 
     * Ví dụ:
     ...
     * 2. Behavioral Profile (implicit signal): Skills + Locations từ job user đã
     * VIEW/SAVE/APPLY
     * 
     * Công thức:
