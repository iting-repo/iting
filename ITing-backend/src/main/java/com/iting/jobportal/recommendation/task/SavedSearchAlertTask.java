...
/**
 * Quét DB tìm các saved-search có alerts enabled, tìm job match mới hơn lần gửi trước, gửi email
 * digest và update timestamp.
 *
 * <p>Lịch chạy: mỗi giờ. Bên trong sẽ check `alertFrequency`:
 *
 * <ul>
 *   <li>DAILY → chỉ gửi nếu lần gửi trước > 24h
 *   <li>WEEKLY → chỉ gửi nếu lần gửi trước > 7 ngày
 * </ul>
 */
@Component
...
public class SavedSearchAlertTask {
  ...
  private void processSearch(SavedSearch search) {
    ...
    List<Job> newJobs =
        jobRepository.findTop5ByStatusAndIdGreaterThanOrderByCreatedAtDesc(
            JobStatus.ACTIVE,
    ...
    String subject =
        String.format("[ITing] %d việc làm mới phù hợp '%s'", newJobs.size(), search.getName());

    StringBuilder body = new StringBuilder();
    body.append("Chào bạn,\n\n");
    body.append("Có ")
        .append(newJobs.size())
        .append(" việc làm mới phù hợp với saved search '")
