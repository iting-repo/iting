...
public class KafkaConfig {
    ...
    @Bean public NewTopic jobEmbeddingTopic() {
        return TopicBuilder.name(topics.getJobEmbeddingRequested()).partitions(3).replicas(1).build();
    }
    @Bean public NewTopic applicationCreatedTopic() {
        return TopicBuilder.name(topics.getApplicationCreated()).partitions(3).replicas(1).build();
    }
    @Bean public NewTopic kybReviewCompletedTopic() {
        return TopicBuilder.name(topics.getKybReviewCompleted()).partitions(3).replicas(1).build();
    }
    @Bean public NewTopic jobEmbeddingDlq() {
        return TopicBuilder.name(topics.dlqOf(topics.getJobEmbeddingRequested())).partitions(1).replicas(1).build();
    }
    @Bean public NewTopic applicationCreatedDlq() {
        return TopicBuilder.name(topics.dlqOf(topics.getApplicationCreated())).partitions(1).replicas(1).build();
    }
    ...
    /**
     * Consumer container with manual ack + DLQ on terminal failures.
     * Retries: 3 attempts with exponential backoff (1s → 2s → 4s), then publish to {topic}.DLT.
     */
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, Object> kafkaListenerContainerFactory(
        ...
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(template,
                (record, ex) -> new org.apache.kafka.common.TopicPartition(record.topic() + topics.getDlqSuffix(), record.partition()));
