...
public class CacheConfig {
  ...
  /**
   * L1: Caffeine. Always available, even when Redis is off. Used as fallback in non-clustered dev
   * or when Redis is not provisioned.
   */
  @Bean
  ...
  public CacheManager caffeineCacheManager() {
    CaffeineCacheManager mgr =
        new CaffeineCacheManager(
            CacheNames.JOB_LIST,
            CacheNames.JOB_DETAIL,
            CacheNames.RECOMMENDATION,
            CacheNames.JOB_EMBEDDING,
            CacheNames.STATIC_CONTENT,
            CacheNames.COMPANY_DETAIL,
            CacheNames.CATEGORY_TREE,
            CacheNames.KNOWLEDGE_GRAPH);
    mgr.setCaffeine(
        Caffeine.newBuilder()
  ...
  /**
   * L2: Redis. Primary when Redis is enabled. Per-cache TTL. Pair with a small Caffeine in front of
   * hot reads if needed (CompositeCacheManager not used here because Spring Cache lookup picks the
   * first hit; we keep Redis as the single distributed source of truth and rely on JVM-level
   * memoization at call site for hottest paths).
   */
  @Bean
  @Primary
  @ConditionalOnProperty(prefix = "app.redis", name = "enabled", havingValue = "true")
  public CacheManager redisCacheManager(
      RedisConnectionFactory cf, GenericJackson2JsonRedisSerializer redisJsonSerializer) {
