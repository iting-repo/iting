...
/**
 * preHandle interceptor luôn return true (không chặn request), nhưng:
 *  - URI matches alias cũ → log warn + 3 response headers
 *    (X-Deprecated-Path, X-Replacement-Path, Deprecation)
 *  - URI không match → không set header gì
 *  - Match đầu tiên thắng (break sau khi tìm thấy)
 */
