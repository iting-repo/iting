 ...
 *
 * <ul>
 *   <li>Refresh token MUST have unique JTI (JWT ID) — enables revocation
 *   <li>Access token MUST NOT be usable as refresh token (type claim check)
 *   <li>Refresh secret MUST be different from access secret
 *   <li>Expiry is enforced on refresh tokens
 *   <li>Tampered refresh tokens are rejected
 * </ul>
 */
...
class RefreshTokenSecurityTest {
  ...
  class TokenIdUniquenessTests {
    ...
    void refreshToken_shouldHaveUniqueJti() {
      ...
      assertEquals(
          count,
          jtis.size(),
          "JTI collision detected — refresh tokens not unique. " + "Revocation list would fail.");
    }
    ...
    void refreshToken_shouldHaveValidJti() {
      ...
      assertTrue(
          jti.matches("[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}"),
  ...
  class TokenTypeIsolationTests {
    ...
    void tokenWithoutTypeClaim_shouldBeRejectedAsRefresh() {
      ...
      try {
        ...
        assertFalse(
            refreshTokenUtil.validateRefreshToken(tokenWithRefreshSecret),
  ...
  class TamperingTests {
    ...
    void tamperedUserIdClaim_shouldBeRejected() {
      ...
      assertFalse(
          refreshTokenUtil.validateRefreshToken(tampered),
