 ...
 *
 * <ul>
 *   <li>BCrypt cost (work factor) ≥ 10 — strong enough for 2026
 *   <li>Salt uniqueness — same password → different hash
 *   <li>Verification correctness
 *   <li>Timing-safe comparison (no early-return leak)
 *   <li>Hash length & format (`$2a$10$...`)
 *   <li>Long & unicode password support
 * </ul>
 */
...
class PasswordSecurityTest {
  ...
  class HashingStrengthTests {
    ...
    void bcryptCost_shouldBeAtLeast10() {
      ...
      assertTrue(
          cost >= 10,
          "BCrypt cost must be ≥ 10 (current: "
              + cost
              + "). "
    ...
    void hashLength_shouldBe60Chars() {
      String hash = encoder.encode("Password123!");
      assertEquals(60, hash.length(), "BCrypt hashes must be exactly 60 chars");
    }
  }
  ...
  class SaltUniquenessTests {
    ...
    void manyHashesOfSamePassword_shouldAllBeUnique() {
      ...
      assertEquals(100, hashes.size(), "BCrypt produced duplicate hashes — salt may not be random");
    }
  }
  ...
  class VerificationTests {
    ...
    @Test
    @DisplayName("Matches returns false for empty/null mismatch")
    void matches_emptyOrNull_shouldReturnFalse() {
      String hash = encoder.encode("Password123");

      assertFalse(encoder.matches("", hash));
      // Spring Security BCryptPasswordEncoder throws on null rawPassword; treat as "no match"
  ...
  class TimingSafeTests {
    ...
    void matchingTime_shouldNotLeakViaTimingSideChannel() {
      ...
      assertTrue(ratio < 5.0, "Timing variance suggests potential side-channel. Ratio: " + ratio);
    }
  }
  ...
  class EdgeCaseTests {
    ...
    void hashShouldNotLeakPassword() {
      ...
      assertFalse(hash.contains(password), "BCrypt hash leaked plaintext password!");
