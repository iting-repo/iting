...
class JobAlertServiceImplTest {
    ...
    void getJobsFromFollowedCompanies_withOneFollowAndActiveJob_returnsJob() {
        ...
        when(jobRepository.findByCompany_IdAndStatus(eq(10L), eq(JobStatus.ACTIVE), any(Pageable.class)))
    ...
    void getJobsFromFollowedCompanies_aggregatesJobsFromMultipleCompanies() {
        ...
        Company c1 = new Company(); c1.setId(10L);
        Company c2 = new Company(); c2.setId(20L);

        Job job1 = new Job(); job1.setId(100L); job1.setStatus(JobStatus.ACTIVE); job1.setCompany(c1);
        Job job2 = new Job(); job2.setId(200L); job2.setStatus(JobStatus.ACTIVE); job2.setCompany(c2);

        when(userFollowCompanyRepository.findByUserId(eq(1L), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(follow1, follow2)));
        when(jobRepository.findByCompany_IdAndStatus(eq(10L), eq(JobStatus.ACTIVE), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(job1)));
        when(jobRepository.findByCompany_IdAndStatus(eq(20L), eq(JobStatus.ACTIVE), any(Pageable.class)))
