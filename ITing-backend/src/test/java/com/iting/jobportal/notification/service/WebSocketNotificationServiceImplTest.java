...
class WebSocketNotificationServiceImplTest {
  ...
  @InjectMocks private WebSocketNotificationServiceImpl wsNotificationService;
  ...
  class SendNotificationTests {
    ...
    void sendNotificationToRecipient_company_sendsToCorrectDestination() {
      ...
      verify(messagingTemplate)
          .convertAndSend(eq("/topic/company/2/notifications"), eq(notification));
    }
    ...
    void sendNotificationToRecipient_admin_sendsToCorrectDestination() {
      ...
      verify(messagingTemplate)
          .convertAndSend(eq("/topic/admin/1/notifications"), eq(notification));
    }
  ...
  class SendUnreadCountTests {
    ...
    void sendUnreadCount_user_sendsCorrectPayload() {
      ...
      verify(messagingTemplate)
          .convertAndSend(eq("/topic/user/1/notifications/unread-count"), payloadCaptor.capture());
    ...
    void sendUnreadCount_company_sendsCorrectPayload() {
      ...
      verify(messagingTemplate)
          .convertAndSend(
              eq("/topic/company/2/notifications/unread-count"), payloadCaptor.capture());
