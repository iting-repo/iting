...
class NotificationPreferenceServiceImplTest {
  ...
  void getOrCreate_existingPreference_returnsExisting() {
    NotificationPreference existing =
        NotificationPreference.builder().id(1L).jobAlerts(false).applicationUpdates(true).build();
  ...
  void getOrCreate_missingPreferenceAndMissingAccount_throws404() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.getOrCreate(99L));
  ...
  void update_partial_onlyAppliesNonNullFields() {
    ...
    NotificationPreferenceDto dto =
        NotificationPreferenceDto.builder().applicationUpdates(false).build();
  ...
  void update_quietHoursEnabledDifferentStartEnd_ok() {
    NotificationPreference existing =
        NotificationPreference.builder().id(1L).quietHoursEnabled(false).build();
    when(repository.findById(1L)).thenReturn(Optional.of(existing));
    when(repository.save(any())).thenAnswer(i -> i.getArgument(0));
  ...
  void update_missingPreference_createsDefaultFirst() {
    ...
    NotificationPreferenceDto dto = NotificationPreferenceDto.builder().jobAlerts(false).build();
  ...
  void update_quietHoursDisabledWithSameTimes_noError() {
    ...
    NotificationPreferenceDto dto =
        NotificationPreferenceDto.builder().quietHoursEnabled(false).build();
