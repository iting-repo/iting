...
class AdminBlogServiceImplTest {
  ...
  void getBlogById_throwsNotFound_whenAbsent() {
    ...
    assertThatThrownBy(() -> service.getBlogById(99L)).isInstanceOf(ResponseStatusException.class);
  }
  ...
  void createBlog_withExplicitSlug_usesProvidedSlug() {
    BlogRequest req =
        BlogRequest.builder().title("Hello World").slug("hello-world").status("PUBLISHED").build();
  ...
  void createBlog_appendsTimestamp_onSlugCollision() {
    BlogRequest req = BlogRequest.builder().title("Duplicate").slug("duplicate").build();
    // First call (exact slug) → collision. The impl may or may not re-check
    // the timestamped slug; mark that stub lenient to allow either branch.
    when(blogRepository.existsBySlug("duplicate")).thenReturn(true);
    org.mockito.Mockito.lenient()
        .when(blogRepository.existsBySlug(argThat(s -> s != null && s.startsWith("duplicate-"))))
        .thenReturn(false);
    when(blogRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
  ...
  void deleteBlog_throwsNotFound_whenAbsent() {
    ...
    assertThatThrownBy(() -> service.deleteBlog(99L)).isInstanceOf(ResponseStatusException.class);
  }
