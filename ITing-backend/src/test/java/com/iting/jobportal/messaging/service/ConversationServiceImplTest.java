...
class ConversationServiceImplTest {
  ...
  class GetByTypeTests {
    ...
    void getConversationsByType_returnsFiltered() {
      Page<Conversation> page = new PageImpl<>(List.of(testConversation));
      when(conversationRepository.findByUserIdAndType(
              eq(1L), eq(ConversationType.USER_USER), any(Pageable.class)))
          .thenReturn(page);
      when(messageRepository.countUnreadByConversationIdAndReceiverId(1L, 1L)).thenReturn(0L);
      when(userRepository.findById(anyLong())).thenReturn(Optional.of(testUser));

      ConversationListResponse result =
          conversationService.getConversationsByType(1L, ConversationType.USER_USER, 0, 20);

      assertNotNull(result);
      verify(conversationRepository)
          .findByUserIdAndType(eq(1L), eq(ConversationType.USER_USER), any(Pageable.class));
    }
  }
  ...
  class GetByIdTests {
    ...
    void getConversationById_notFound_throwsException() {
      ...
      RuntimeException exception =
          assertThrows(
              RuntimeException.class, () -> conversationService.getConversationById(999L, 1L));
    ...
    void getConversationById_notParticipant_throwsException() {
      ...
      RuntimeException exception =
          assertThrows(
              RuntimeException.class, () -> conversationService.getConversationById(1L, 999L));

      assertEquals(
          "Unauthorized: You are not a participant of this conversation", exception.getMessage());
    }
  }
  ...
  class DeleteTests {
    ...
    void deleteConversation_notParticipant_throwsException() {
      ...
      RuntimeException exception =
          assertThrows(
              RuntimeException.class, () -> conversationService.deleteConversation(1L, 999L));
  ...
  class GetBetweenParticipantsTests {
    ...
    void getConversationBetweenParticipants_notExists_returnsNull() {
      ...
      ConversationResponse result =
          conversationService.getConversationBetweenParticipants(1L, 999L);
