...
class CandidateApplicationServiceImplTest {
  ...
  @InjectMocks private CandidateApplicationServiceImpl service;
  ...
  void applyJob_withoutCv_shouldCreateApplyFormAndJoinRecord() {
    ...
    ArgumentCaptor<ApplyFormSentToJob> sentCaptor =
        ArgumentCaptor.forClass(ApplyFormSentToJob.class);
  ...
  void applyJob_whenJobNotFound_shouldThrowNotFound() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.applyJob(1L, request));
  ...
  void applyJob_whenApplyFormSaveFails_shouldNotCreateJoinRecord() {
    ...
    when(candidateApplicationRepository.existsByUserIdAndJobId(1L, 5L)).thenReturn(false);
    when(userRepository.findById(1L)).thenReturn(Optional.of(user));
    when(applyFormRepository.save(any(ApplyForm.class)))
        .thenThrow(new RuntimeException("save form failed"));

    RuntimeException ex = assertThrows(RuntimeException.class, () -> service.applyJob(1L, request));
  ...
  void withdrawApplication_whenApplicationNotFound_shouldThrow() {
    ...
    ResponseStatusException ex =
        assertThrows(ResponseStatusException.class, () -> service.withdrawApplication(1L, 10L));
  ...
  void getMyApplications_shouldUseCorrectPageableAndMapAllItems() {
    ApplyFormSentToJob sent1 =
        ApplyFormSentToJob.builder()
            .id(new ApplyFormSentToJob.ApplyFormSentToJobId(5L, 10L))
            .build();
    ApplyFormSentToJob sent2 =
        ApplyFormSentToJob.builder()
  ...
  void getMyApplications_whenApplyFormMissing_shouldThrow() {
    ...
    RuntimeException ex =
        assertThrows(RuntimeException.class, () -> service.getMyApplications(1L, 0, 10));
  ...
  void getMyApplications_whenSentIdIsNull_shouldThrowNullPointerException() {
    ApplyFormSentToJob sent = ApplyFormSentToJob.builder().id(null).build();

    when(candidateApplicationRepository.findByUserId(eq(1L), any(Pageable.class)))
        .thenReturn(new PageImpl<>(List.of(sent)));

    assertThrows(NullPointerException.class, () -> service.getMyApplications(1L, 0, 10));
  ...
  void getMyApplications_whenPageOrSizeInvalid_shouldThrowIllegalArgumentException() {
    assertThrows(IllegalArgumentException.class, () -> service.getMyApplications(1L, -1, 10));

    assertThrows(IllegalArgumentException.class, () -> service.getMyApplications(1L, 0, 0));

    verifyNoInteractions(
        candidateApplicationRepository, applyFormRepository, applicationMapperUtil);
  }
  ...
  void applyJob_whenJobIdIsNull_shouldThrowNullPointerExceptionFromRepositoryLookup() {
    ...
    when(jobRepository.findById(null))
        .thenThrow(new NullPointerException("jobId is marked non-null but is null"));
  ...
  void applyJob_whenSavedFormIdIsNull_shouldStillCreateJoinRecordWithNullApplyFormId() {
    ...
    ApplyForm savedForm =
        ApplyForm.builder().id(null).userId(1L).applicantName("Candidate").build();
  ...
  void applyJob_whenSavedSentTimeSentIsNull_shouldReturnResponseWithNullTimeSent() {
    ...
    ApplyForm savedForm = ApplyForm.builder().id(11L).userId(1L).build();
  ...
  void withdrawApplication_whenApplyFormUserIdIsNull_shouldThrowNullPointerException() {
    ApplyForm form = ApplyForm.builder().id(10L).userId(null).build();

    when(applyFormRepository.findById(10L)).thenReturn(Optional.of(form));

    assertThrows(NullPointerException.class, () -> service.withdrawApplication(1L, 10L));
  ...
  @Test
  void
      getMyApplications_whenSentIdApplyFormIdIsNull_shouldPassNullToFindByIdThenThrowApplyFormNotFound() {
    ApplyFormSentToJob sent =
        ApplyFormSentToJob.builder()
            .id(new ApplyFormSentToJob.ApplyFormSentToJobId(5L, null))
            .build();
  ...
  void getMyApplications_whenRepositoryReturnsNullPage_shouldThrowNullPointerException() {
    when(candidateApplicationRepository.findByUserId(eq(1L), any(Pageable.class))).thenReturn(null);
  ...
  void hasApplied_whenRepositoryThrows_shouldPropagateException() {
    ...
    RuntimeException ex = assertThrows(RuntimeException.class, () -> service.hasApplied(1L, 5L));
