...
class PasswordResetServiceImplTest {
  ...
  void createPasswordResetToken_existingAccount_savesOtpAndSendsEmail() throws Exception {
    ...
    verify(emailService)
        .sendHtmlEmail(eq("user@test.com"), contains("đặt lại mật khẩu"), anyString());
  }
  ...
  void resetPassword_validOtp_updatesPasswordAndClearsOtp() {
    Account account =
        Account.builder().id(1L).email("user@test.com").passwordHash("OLD_HASH").build();
    OtpCode otp =
        OtpCode.builder()
            .email("user@test.com")
            .code("123456")
    ...
    when(otpCodeRepository.findTopByEmailAndIsVerificationOrderByExpiryTimeDesc(
            "user@test.com", false))
    ...
    assertEquals(
        "NEW_HASH", account.getPasswordHash(), "Password hash phải được update sang hash mới");
  ...
  void resetPassword_noOtpFound_throws() {
    when(otpCodeRepository.findTopByEmailAndIsVerificationOrderByExpiryTimeDesc(
            eq("u@test.com"), eq(false)))
        .thenReturn(Optional.empty());

    RuntimeException ex =
        assertThrows(
            RuntimeException.class, () -> service.resetPassword("u@test.com", "123456", "newpass"));
  ...
  void resetPassword_wrongOtp_throws() {
    OtpCode otp =
        OtpCode.builder().code("111111").expiryTime(LocalDateTime.now().plusMinutes(5)).build();
    when(otpCodeRepository.findTopByEmailAndIsVerificationOrderByExpiryTimeDesc(
            anyString(), eq(false)))
        .thenReturn(Optional.of(otp));
    ...
    RuntimeException ex =
        assertThrows(
            RuntimeException.class, () -> service.resetPassword("u@test.com", "999999", "newpass"));
  ...
  void resetPassword_expiredOtp_throws() {
    OtpCode otp =
        OtpCode.builder().code("123456").expiryTime(LocalDateTime.now().minusMinutes(1)).build();
    when(otpCodeRepository.findTopByEmailAndIsVerificationOrderByExpiryTimeDesc(
            anyString(), eq(false)))
        .thenReturn(Optional.of(otp));
  ...
  void resetPassword_accountNotFound_throws() {
    OtpCode otp =
        OtpCode.builder().code("123456").expiryTime(LocalDateTime.now().plusMinutes(5)).build();
    when(otpCodeRepository.findTopByEmailAndIsVerificationOrderByExpiryTimeDesc(
            anyString(), eq(false)))
        .thenReturn(Optional.of(otp));
  ...
  void resetPassword_emailNormalization_trimAndLowercase() {
    Account account = Account.builder().id(1L).email("user@test.com").passwordHash("OLD").build();
    OtpCode otp =
        OtpCode.builder().code("123456").expiryTime(LocalDateTime.now().plusMinutes(5)).build();
    when(otpCodeRepository.findTopByEmailAndIsVerificationOrderByExpiryTimeDesc(
            eq("user@test.com"), eq(false)))
        .thenReturn(Optional.of(otp));
    when(accountRepository.findByEmail("user@test.com")).thenReturn(Optional.of(account));
    ...
    verify(otpCodeRepository)
        .findTopByEmailAndIsVerificationOrderByExpiryTimeDesc("user@test.com", false);
