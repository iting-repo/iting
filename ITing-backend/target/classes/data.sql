...
INSERT INTO roles (name, description, created_at, updated_at) VALUES
                                                                  ...
                                                                  ('EMPLOYER', 'Company employer', NOW(), NOW()),
                                                                  ('CANDIDATE', 'Job seeker', NOW(), NOW());

-- ===== PERMISSIONS =====
INSERT INTO permissions (code, name, module, action, sort_order, active, created_at, updated_at) VALUES
                                                                                                     ('USER_VIEW', 'View users', 'USER', 'READ', 1, TRUE, NOW(), NOW()),
                                                                                                     ('USER_MANAGE', 'Manage users', 'USER', 'WRITE', 2, TRUE, NOW(), NOW()),
                                                                                                     ('JOB_CREATE', 'Create job', 'JOB', 'CREATE', 3, TRUE, NOW(), NOW()),
                                                                                                     ('JOB_VIEW', 'View job', 'JOB', 'READ', 4, TRUE, NOW(), NOW()),
                                                                                                     ('JOB_APPLY', 'Apply job', 'JOB', 'APPLY', 5, TRUE, NOW(), NOW());
...
WHERE r.name = 'EMPLOYER'
  AND p.code IN ('JOB_CREATE', 'JOB_VIEW');

INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r, permissions p
WHERE r.name = 'CANDIDATE'
  AND p.code IN ('JOB_VIEW', 'JOB_APPLY');

-- ===============================================
-- ACCOUNTS
-- password = 123456 (bcrypt sample)
-- ===============================================

INSERT INTO accounts (email, password_hash, status, created_at, updated_at) VALUES
                                                                                ('admin@iting.com', '$2a$10$Dow1exampleHash123456', 'ACTIVE', NOW(), NOW()),
                                                                                ('hr@fpt.com', '$2a$10$Dow1exampleHash123456', 'ACTIVE', NOW(), NOW()),
                                                                                ('candidate1@gmail.com', '$2a$10$Dow1exampleHash123456', 'ACTIVE', NOW(), NOW());
...
SELECT a.id, r.id
FROM accounts a, roles r
WHERE a.email = 'hr@fpt.com'
  AND r.name = 'EMPLOYER';

INSERT INTO account_roles (account_id, role_id)
SELECT a.id, r.id
FROM accounts a, roles r
WHERE a.email = 'candidate1@gmail.com'
